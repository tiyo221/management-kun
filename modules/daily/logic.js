...
(function () {
  ...
  function loadSample() {
    ...
    // 開始時刻はユーザ設定なので保つ（サンプルが差し替えるのは項目とルーチンだけ）。既定へ戻すと、
    // 開始時刻だけ変えて項目がまだ 0 件の状態＝空判定でサンプル投入バーが出る場面（初回に最も
    // 起こりやすい）で、投入した瞬間に設定が黙って 09:00 へ戻る。
    save({ version: 1, startTime: startTime(), routines: [
