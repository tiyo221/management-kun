/* モジュール dashboard（プロジェクト・ダッシュボード＝横断集約ビュー）— ビュー（描画・遷移）。集約計算は MK.logic.dashboard に委譲。CONVENTIONS §1
   ...
   アサイン状況・関連プロダクトを1画面で一望する。読み取り専用の集約に留め、編集はこの画面では行わない（更新にあたる
   主操作を持たない ── spec/modules/dashboard.md）。遷移先がモジュールのカード（WBS 進捗・アサイン状況）には導線を
   置き（ctx.route）、遷移先がマスタのカード（基本情報・関連プロダクト）はシェルのナビから開けるので重ねない。
   データが無い領域は空状態の案内を出す（Issue #78）。 */
(function () {
  "use strict";
  const MK = window.MK;
  const el = MK.util.el;
