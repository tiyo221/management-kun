...
- **自分**: ✅ ToDo（GTD）／ 🎯 目標（ロードマップ・ダッシュボード）／ ❓ わからないこと
- **ピープル**: 📊 スキル（マトリクス・ギャップ）／ 🧑‍🤝‍🧑 リソース（要員計画・横断）／ 💬 1on1メモ
- **デリバリー**: 🗂 WBS（階層・ガント・依存。プロジェクト単位）
- **プロダクト**: 🚀 リリース台帳（リリース／ロードマップ）
- **テクノロジー**: 🧰 技術スタック台帳
- **マスタ**: 👤 人 ／ 📁 プロジェクト ／ 📦 プロダクト（全モジュール横断で参照する共通マスタ）
- ⚙ 設定: 全体バックアップ（JSON 入出力）・サンプル投入・旧ツール移行・テーマ切替（ダークモード）・モジュールの表示/非表示切替

> 負荷（📈 workload）は登録・データを維持したままナビから降格（非表示）。要員計画（リソース）が計画のハブを担う。CSV 入出力は skills / wbs / techstack / todo / workload とマスタ（人 / プロジェクト / プロダクト）に対応。
