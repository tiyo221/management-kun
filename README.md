...
**モジュールを試す**: まだデータの無いモジュールを開くと「サンプルを入れて試す」バーが出る。押すと**そのモジュールだけ**にサンプルが入り、「サンプルを片付ける」で元の空の状態へ戻せる（他モジュール・マスタは巻き込まない。spec §3.6.2）。

全体をまとめてサンプルで埋めたいときは「設定」→「サンプルデータを読み込む」（人・プロジェクト・プロダクトのマスタと全モジュールを**置き換える**ので、実データがあるときは使わない）。旧ツールのデータがあれば「設定」→「旧ツールから移行」で取り込める。
