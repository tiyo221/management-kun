...
- **代入形で対象外にするもの**: 値が ASCII でない（`token: 認証に使う文字列` のような**日本語の散文**）／値が識別子の連なり（`process.env.API_KEY` / `settings.apiKey` ＝**リテラルが書かれていないコード**）。これは「中身がダミーか」の推測ではなく「そもそも値が書かれていない」判定なので、暗黙免除には当たらない。
...
- **抑止（逃げ道）**: 行に `secrets-allow` を含めるとその行は検査しない。理由を併記する。
  ...
  例外は差分に出てレビューでき、`git grep -c secrets-allow` で総数を数えられる。**「緩める」＝例外を1つ足すことであって、規則を弱めることではない。**
...
- **落ちる条件（混入以外）**: `git ls-files` が使えない（追跡状態を判定できない環境ではこの検査が成立しないため、素通り＝緑にせず落とす）。
