...
- **検出するもの**: 既知のキー接頭辞（`sk-` / `ghp_` 等の GitHub トークン / `AKIA` / Slack / Google / 秘密鍵ヘッダ）、JWT、32文字以上の16進値、`.env` 形式（`KEY=長い値`）、代入形（`password` / `api_key` / `secret` / `token` に 8 文字以上の値。**引用符の有無を問わない** ── JSON も `.env` もシェルも止めるため）、ローカル絶対パス（`C:\Users\<名前>` / `C:/Users/<名前>` / `/Users/<名前>` / `/home/<名前>`。**区切りは `\` と `/` の両方**を見る。MCP・エディタの JSON 設定や Node の出力は Windows でもスラッシュ区切りで出る）。
