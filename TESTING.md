...
| `*.md`（仕様・ガイドの追加/移動/改名） | ドキュメント間の相対リンク。**インラインリンク形式のみ**が対象で、外部 URL・アンカー・参照定義形式（`[label]: path`）・`<a href>`・コードフェンス内／インラインコード内の記述・リポジトリ外を指す相対リンク（旧ツールのリポジトリ参照）は検査しない（4スペースインデントのコードブロックは検査される） | 自動（`spec-consistency`＝リンク切れ検出・#241） |
