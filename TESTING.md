...
- **検出しないもの**: 説明用のダミー値（`api_key: "YOUR_KEY_HERE"` のような全大文字＋アンダースコア、英小文字だけの `xxxxxxxx`、`your` / `example` / `dummy` / `changeme` 等を含む値）。ドキュメントに例を1行書いただけでフルスイートが赤くなると、回避手段が「共通の正規表現を書き換える」しか無くなり検査ごと外されるため。**例を書くときは値をこの形にする。**
- **検出しないもの**: 高エントロピー文字列の汎用検出。フルスイートは毎回走るため、偽陽性が出ると「とりあえず無視する」運用に堕ちて規約ごと形骸化する（取りこぼしより害が大きい）。各パターンは「接頭辞＋十分な長さの本体」を要求するので、ドキュメントが接頭辞に言及しても落ちない。
