...
- **検査するリンク**: インラインリンク形式の相対パス。宛先の山括弧記法は剥がし、タイトル（`"…"` / `'…'` / `(…)`）は宛先に含めない。`path#anchor` はファイル部分のみ見る。
- **検査しないもの**: 外部 URL・`mailto:`・ページ内アンカー・参照定義形式（`[label]: path`）・`<a href>`・コードフェンス内／インラインコード内の記述・リポジトリ外を指す相対リンク（旧ツールのリポジトリ参照）。4スペースインデントのコードブロックは**検査される**。
- **リンク切れ以外に落ちる条件**: ルート絶対リンク（`/` 始まり。GitHub ではサイトルートに解決され必ず壊れる）と、コードフェンスの閉じ忘れ（以降の本文が丸ごと検査対象外になるため）。
