...
- **検出するもの**: 既知のキー接頭辞（`sk-` / `ghp_` 等の GitHub トークン / `AKIA` / Slack / Google / 秘密鍵ヘッダ）、引用符で囲まれた値を伴う代入形（`password` / `api_key` / `secret` / `token`）、ローカル絶対パス（`C:\Users\<名前>` / `C:/Users/<名前>` / `/Users/<名前>` / `/home/<名前>`）。**区切りは `\` と `/` の両方**を見る（MCP・エディタの JSON 設定や Node の出力は Windows でもスラッシュ区切りで出るため）。
- **検出しないもの（ダミー値）**: 説明用の値。全大文字＋アンダースコア（`YOUR_KEY_HERE`）、英小文字だけ（`xxxxxxxx`）、記号の反復（`******`）、`your` / `example` / `sample` / `dummy` / `placeholder` / `changeme` / `redacted` を**語として**含む値、`foo` / `bar` / `test` / `todo` などの汎用語は**値全体がその語＋数字**のときだけ。語の部分一致では免除しない（`Rhubarb2024!` の `bar` で本物を素通りさせないため）。ドキュメントに例を1行書いただけでフルスイートが赤くなると、回避手段が「共通の正規表現を書き換える」しか無くなり検査ごと外されるため。**例を書くときは値をこの形にする。**
- **検出しないもの（高エントロピー）**: 文字列のランダム性による汎用検出。フルスイートは毎回走るため、偽陽性が出ると「とりあえず無視する」運用に堕ちて規約ごと形骸化する（取りこぼしより害が大きい）。各パターンは「接頭辞＋十分な長さの本体」を要求するので、ドキュメントが接頭辞に言及しても落ちない。
