...
- **リンク切れ以外に落ちる条件**: ルート絶対リンク（`/` 始まり。GitHub ではサイトルートに解決され必ず壊れる）、バックスラッシュ区切り（GitHub 上では 404。OS によって結果が割れないよう常に落とす）、2段以上上を指す相対リンク（`../` の数の書き間違いとみなす）、コードフェンスの閉じ忘れ（以降の本文が丸ごと検査対象外になるため）、追跡されているのに作業ツリーに無い md（`git rm` せず消した／移動した）、**リンク先が Git 追跡下に無い**（`.gitignore` 済み・コミット忘れ。作業ツリーにあっても GitHub 上では 404 になるため）。
