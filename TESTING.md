...
- **リンク切れ以外に落ちる条件**: ルート絶対リンク（`/` 始まり。GitHub ではサイトルートに解決され必ず壊れる）、2段以上上を指す相対リンク（`../` の数の書き間違いとみなす）、コードフェンスの閉じ忘れ（以降の本文が丸ごと検査対象外になるため）、追跡されているのに作業ツリーに無い md（`git rm` せず消した／移動した）。
