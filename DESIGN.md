...
- Don't use native browser dialogs (`confirm()` / `alert()` / `prompt()`) — they ignore the design tokens and dark mode. Use the shared overlays instead: `MK.ui.undoDeleteToast` for destructive actions (undo is the default — deletes run immediately and offer an undo toast, reachable by click or `Ctrl+Z` / `⌘Z` while it is shown), `MK.ui.confirm` only for irreversible ones, `MK.ui.toast` for notices, `MK.ui.modal` for input or richer dialogs (see [`CONVENTIONS.md`](CONVENTIONS.md) §2.3 / §2.5-3 / `spec.md` §6).
...
- Specific dark-mode token values not surfaced beyond hero bands（マネジメントくんでは `[data-theme="dark"]` の独自派生配色を `shared/design.css` に定義。spec §6.2）
