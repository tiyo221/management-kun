...
2. **編集はインライン既定** — 一覧項目の編集はその場で行う（クリックで入力欄化、Enter 確定・Esc 取消）。モーダルを使ってよいのは「3項目以上を同時に変える」「新規作成で必須項目が複数ある」ときだけ。共通ヘルパを使い、各モジュールで自作しない（ヘルパ本体は最初の消費者と同時に `shared/ui.js` へ入れる。#244 の todo が1件目。[`CODING.md`](CODING.md)「共通化は2か所以上で必要になってから」）。付随して次を守る:
   ...
   - **行内の編集は logic の戻り値を見て、掴んだ実体が消えていたら受け入れない**。CSV / JSON 取込や別画面の削除で行だけが画面に残ることがある。更新系（`updateXxx`）は削除系（`removeXxx`）と同じく**空振りなら false / null を返して保存もしない**契約にし（空振りの保存は直前の削除の退避＝取り消しを潰す）、view は元値へ戻したうえで**消えた事実をトーストで伝えて再描画する**（無言の no-op にしない。§2.5-3 と同じ理由）。**この契約を満たすのは skills / oneonone / releases / metrics で、先行してインライン化した techstack / questions / goals の `updateItem` / `updateGoal` / `updateStep` は戻り値を持たない** ── 冒頭の遡及規定のとおり、触ったときに揃える（上の項目で「`updateItem` で足りる」と言っているのは空必須項目の拒否の話で、こちらの契約とは別）。
