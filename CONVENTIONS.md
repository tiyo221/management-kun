# CONVENTIONS.md — UI・レイアウト規約 ＋ モジュール・アーキテクチャ規約 ＋ 完成の点検（DoD）
...
- 使い方: モジュールを追加・修正するときは、着手前に §1〜§4 を読み、完了前に §6 で自己点検する（§6 は担保者ごとに3つ ── 機械＝`node test/run.js`／規約＝§1〜§5 の読み返し／人の目視＝[`TESTING.md`](TESTING.md) §3.1）。共通の不具合・要望は、まず §4 の共有資産で直せないかを検討する。
...
1. **主操作は1アクション** — 各モジュールには「一番多く繰り返す操作」が1つある（todo＝Inbox の仕分け、daily＝時間の割当、skills＝評価の入力）。**数えるのはデータを変える操作**（表示の切替・他モジュールへの遷移は含めない ── 含めると読むための画面にも形だけの主操作が立ち、宣言が設計を縛らなくなる）。これを個別仕様 [`spec/modules/<id>.md`](spec/modules/) に `## 主操作` として明記し、**1アクションで完了させる**。主操作にモーダルを挟まない。**節の有無は機械が見る**（§6.1。導入時点で欠けていた既存分だけ `test/spec-consistency.test.js` の `MAIN_ACTION_PENDING` に明示して外してある ── 埋めたらその一覧からも消す）。**「主操作を持たない」と宣言してよいのは、自前の永続データを持たず更新操作が存在しない読み取り専用の集約ビューだけ**（dashboard・#324）。その場合も節は立て、根拠（永続データを持たないこと・編集がどのモジュールにあるか）と、代わりに §2.5-5 の最上段の基準として何を採るかを書く ── 機械は見出しと中身の有無しか見ないので、条件を書かずに「持たない」を許すと1行書けば検査を通せる抜け道になる。
2. **編集はインライン既定** — 一覧項目の編集はその場で行う（クリックで入力欄化、Enter 確定・Esc 取消）。モーダルを使ってよいのは「3項目以上を同時に変える」「新規作成で必須項目が複数ある」ときだけ。共通ヘルパを使い、各モジュールで自作しない（ヘルパ本体は最初の消費者と同時に `shared/ui.js` へ入れる。#244 の todo が1件目。[`CODING.md`](CODING.md)「共通化は2か所以上で必要になってから」）。付随して次を守る:
   - **タイトルのクリックはインライン編集が取るので、モーダルへの導線は明示のボタンにする**（「編集」「詳細」）。行全体のクリックでモーダルを開いていたモジュールは、インライン化と同時にボタンへ移す。
   - **行内へ出した値は chip でも重ねて表示しない**（ステータス select を置いたらステータス chip は消す）。同じ情報が2か所にあると、片方だけ更新する事故が起きる。
   - **空必須項目の拒否は view に置く**（`onCommit` が `false` を返す）。**logic に専用 setter を足すのは、更新に副作用があるときだけ**（daily の `setTitle` は todo へ書き戻すので logic 側。techstack / questions / goals は公開済みの `updateItem` / `updateGoal` で足りるので view 側）。判定のためだけに `updateXxx` の薄いラッパを増やさない。
   - **インライン編集で確定したら、行が握っているオブジェクトにも書き戻す**。いまは `MK.store` が `_cache` を持ち `load()` が同じオブジェクトを返すため logic 側の更新が見えており、書き戻しは冗長になっている。それでも書くのは**キャッシュの共有に依存しないため** ── ここが変わった瞬間に、直後の削除トーストやモーダルの初期値が旧い値になる形で壊れる（壊れ方が「たまに古い文字列が出る」なので気づきにくい）。
   - **行内の編集は logic の戻り値を見て、掴んだ実体が消えていたら受け入れない**。CSV / JSON 取込や別画面の削除で行だけが画面に残ることがある。更新系（`updateXxx`）は削除系（`removeXxx`）と同じく**空振りなら false / null を返して保存もしない**契約にし（空振りの保存は直前の削除の退避＝取り消しを潰す）、view は元値へ戻したうえで**消えた事実をトーストで伝えて再描画する**（無言の no-op にしない。§2.5-3 と同じ理由）。**この契約を満たすのは skills（`updateSkill`）/ oneonone（`updateAction`）/ releases（`updateRelease`）/ metrics（`updateMetric`）で、先行してインライン化した techstack / questions / goals の `updateItem` / `updateGoal` / `updateStep` と、oneonone の `updateEntry` は戻り値を持たない**（関数単位で見る ── 同じモジュールでも満たす関数と満たさない関数が混在する。満たさない関数の戻り値で分岐すると、成功時も falsy なので毎回「消えている」と誤判定する） ── 冒頭の遡及規定のとおり、触ったときに揃える（上の項目で「`updateItem` で足りる」と言っているのは空必須項目の拒否の話で、こちらの契約とは別）。
   - **絞り込みのある一覧で新規追加するときは、絞り込みを追加先へ寄せてから再描画する**（techstack のクイック追加は ring 固定なので、別タブで絞り込み中だと追加行が見えない）。
3. **破壊的操作は undo 既定** — 削除は確認を出さず即実行し、取り消しトースト `MK.ui.undoDeleteToast(message, tryUndo, onRestored)`（`MK.ui.undoToast` の定型。失敗時の文言もここで1本化する）を出す。`MK.ui.confirm` を使ってよいのは**取り消し不能な操作だけ**（全データ削除・インポートの置換・旧データ移行）。undo の対象は**削除だけ**に限る（編集・ステータス変更は元に戻す操作が自明。logic 側は「直前に消した1件」だけ保持すればよく、汎用 undo スタックは [`CODING.md`](CODING.md) のオーバーエンジニアリング防止と衝突する）。**退避した1件は、他の変更が入った時点で破棄する**（別の変更で位置がずれる／scoped モジュールは対象切替でデータセットごと変わるため、古い退避を戻すとデータが壊れる）。ここで言う「他の変更」は**ユーザーが入れた変更**を指す。画面を開いただけで走る自動処理（daily のルーチン自動投入など）の保存で退避を潰さない ── 潰すと削除直後の再描画がそれを踏み、出したばかりの取り消しトーストが必ず失敗する。ただし退避した位置がずれない足し方（末尾に足す等）であることを確かめてから例外にする。**退避を持つ logic は `forgetUndo()` を生やす**（退避を復元せず捨てる）── store を logic の外から書き換える経路（全データ初期化 `MK.store.clearAll()`）は保存を通らないので、そこから捨てないと初期化後の `Ctrl+Z` で1件だけ復活する。捨てる側は `MK.forgetAllUndo()`（マスタ＋`forgetUndo()` を持つ全 logic を舐める）を呼ぶ。トースト側も同時に1つしか出さない（`MK.ui.undoToast` が保証済み）。破棄後に「元に戻す」が押されうるため、**logic の undo は復元できたかを返し、view は戻せなかったことを伝える**（無言の no-op にしない。伝える文言は `undoDeleteToast` が持つので view で書かない）。トースト表示中は `Ctrl+Z`（Mac: `⌘Z`）でも取り消せる（キーボードのある環境では本文にもそう表記する。タッチのみの端末では到達できない案内になるため出さない。[`spec.md`](spec.md) §10.2「キーボードで到達可能」。テキスト入力中は横取りしない・フォーカスがトースト内にある間は自動消滅しない ── **この到達性は `MK.ui.undoToast` が担保するので、各モジュールで undo UI を自作しない**）。
4. **行内で完結する操作は行だけ更新する** — チェック・ステータス変更・インライン編集は該当行のみ差し替え、`render()` で画面全体を作り直さない（スクロール位置・フォーカスを失わないため）。**部分更新のために掴んだノードは `unmount()` で全部手放す**（一覧の器・件数バッジ・見出しの数字など。1つでも残すと、インライン編集中にモジュールを切り替えたとき blur の確定が走り、画面に無いノードへ書き込む）。**この手放しは機械が見る**（§6.1。view のモジュールスコープにある `root` / `…Node` / `…Host` / `…El` / `…ById` / `ui.countBadges()` の束が対象で、`unmount()` での `null` / `undefined` / `{}` / `[]` 代入か `.clear()` を求める）。手放さないのが正しい持ち方（モーダルの寿命に紐づき `onClose` で捨てる参照・#265）は、宣言行に `// unmount 不要: 理由` を書いて外す。**行を丸ごと作り直すのは描き方の種類が変わるときだけにし、作り直したら操作中だったコントロールへフォーカスを戻す**（毎回作り直すと select ごと DOM から外れ、キーボードで操作を続けられない。[`spec.md`](spec.md) §10.2）。フィルタ変更・タブ切替・データ取込など**画面の意味が変わる操作は全再描画でよい**（全再描画という割り切りが view を小さく保っている。差分管理を各 view に持たせるとミニフレームワークの内製になる）。
5. **画面上部は使用頻度順** — 最上段には主操作に必要なものだけを置く。CSV 入出力・サンプル投入など低頻度の操作は画面最下部へ置く。

> **既存モジュールへの遡及適用は求めない**（1〜5 に共通）。規約を置いた時点では既存モジュールの大半が未適合（削除は `MK.ui.confirm`、編集はモーダル）だが、一斉移行のキャンペーンは張らない。**触ったモジュールを §6 の DoD で規約へ合わせる**ことで展開する。
...
- オーバーレイ: `modal({title, body, actions:[{label,variant,onClick(close)}], onClose, persistent})`（`onClose` はどう閉じても＝アクション・Esc・overlay クリック・一括クローズでも1度だけ呼ぶ。モーダルの寿命に紐づく参照はここで手放す。**開いたモーダルはシェルがビュー切替時に `MK.ui.closeAllModals()` で畳むので、`unmount` で手書きに閉じない**。`persistent: true` はその一括クローズから外す指定で、画面に紐づかない案内＝保存失敗の警告にだけ使う。Issue #265） / `closeAllModals()`（開いているモーダルを畳む。**呼ぶのはシェルのビュー切替だけ**。同じビューを組み直すだけの再描画（マスタ更新・絞り込み切替）では畳まない ── 畳むと編集中モーダルの入力途中が消えるため。モジュールからは呼ばない。`_resetModals()` は `persistent` も含めて畳むテスト専用フックで、view からは呼ばない） / `toast(message, type)` / `undoToast(message, onUndo)`（削除の取り消し。6秒表示・「元に戻す」または `Ctrl+Z` で `onUndo`。§2.5-3） / `undoDeleteToast(message, tryUndo, onRestored)`（削除の定型。`tryUndo` は復元可否を返す logic 側の undo＝マスタの `undoRemove` / モジュールの `undoDelete`。false なら共通文言で失敗を伝え、true なら `onRestored`（省略可）で再描画する。**削除の取り消しはこれを使い、`undoToast` を直に呼ばない**） / `confirm(message)→Promise<bool>` / `rowMenu(anchor, items=[{label,onClick,danger}])`（一覧行の低頻度操作を寄せる `⋯` ポップアップ。`items` に `null` を混ぜてよい＝条件で出し分ける呼び出し側のため。開けるのは同時に1つ・外側クリック / Esc・項目の実行で閉じ、`onClick` は閉じたあとに呼ぶ。`closeAllModals()` からも畳む。閉じるだけなら `closeRowMenu()`＝再描画・`unmount` の後始末に使う。§2.5-1 の「主操作を行に残す」の裏返しで、**行に常設すると主操作が埋もれる操作をここへ寄せる**。Issue #156 / #266）
- 部分更新: `countBadges()→{make(key,count), refresh(countsMap), clear()}`（タブに添える件数バッジの束。行操作でタブを作り直さず数字だけ差し替える。`render` で `make`、行操作後に `refresh(L().counts())`、`unmount` で `clear()`。§2.5-4 / Issue #299）
...
7. §6 で点検（§6.1 は `node test/run.js`、§6.2 は §1〜§5 の読み返し、§6.3 は [`TESTING.md`](TESTING.md) §3.1 の手動スモーク）。
...
## 6. モジュール完成の点検（Definition of Done）

DoD が持つ固有の情報は「**いつ確かめるか**」だけで、規約そのものは §1〜§5 にある。かつては28項目を1つのリストに並べていたが、**索引（一望する）と出荷ゲート（毎回1つずつ確かめる）を兼務させると、ゲートとしては多すぎて実際には数項目しか見られない**（#312）。そこで項目を担保者ごとに3つへ分けた。

**規約そのものは分けていない。** アクター（機械／人）で割れるのは点検手順だけで、規約は全て本書に残す（2ファイルに割ると規約を探すのに2か所見ることになり、現状より悪くなる）。

### 6.1 機械が担保する — `node test/run.js` が緑であること
...
| 部分更新で掴んだノードを `unmount()` で手放している（§2.5-4） | modules | 〃 |
...
### 6.2 規約として満たす — 読み返す先（項目を再掲しない）

設計として満たすもの。ここに項目を書き写すと §1〜§5 との二重記載になり、必ず片方が古くなる。**完了前に該当節を読み返す**。

- **§1.3 責務の分担** — view に業務計算を置かない／logic の公開関数に JSDoc／マスタは `MK.people` / `MK.projects` 経由／scoped モジュールは `ctx.scope` の対象内に閉じる（[`spec.md`](spec.md) §3.7）。
- **§2.1 余白・リズム／トークン** — 隣接ブロックが密着しない（`.card` 直下・末尾アクションの落とし穴に注意）。色・余白・角丸・タイポは [`DESIGN.md`](DESIGN.md) のトークン経由で、値を直書きしない（このうち機械が見るのは §6.1 のとおり色と余白だけ。**角丸・タイポはここで読み返して満たす** ── §6.3 の目視リストには入れない）。
- **§2.4 安全** — ユーザー入力・取込データは `textContent` かエスケープ。
- **§2.5 操作コスト** — 主操作1アクション／インライン編集既定／undo 既定／行内は行だけ更新／最上段は使用頻度順。
- **§4 共有資産** — 部品を自作せず `MK.ui` のヘルパを使う。**§5 追加手順** — 個別仕様と §5 表。
- **アクセシビリティ・キーボード**（[`spec.md`](spec.md) §10.2） — キーボードで到達可能・読み上げ名の付与。実際に触って確かめる部分は §6.3。
- **横断表示・集約ビュー**（spec §9.5） — 相手をハード参照せず `MK.readSummary` 等の任意契約で問い合わせ、欠損時は枠を黙って省く。
- **データ規格**（[`spec.md`](spec.md) §4） — 日時は ISO 8601 UTC ／日付は `YYYY-MM-DD` ／未設定は `null` / `[]` / `""`。`exportData` / `importData`（置換・マージ）／`loadSample` は自分の namespace だけを扱い、scoped は対象 id を末尾の任意引数で受ける（[`spec.md`](spec.md) §3.5）。
- **ドキュメントの鮮度**（[`CLAUDE.md`](CLAUDE.md) 必須ルール5） — 参照したドキュメントとのズレを放置しない。モジュール id を新たに列挙しない（一覧は [`spec.md`](spec.md) §5 の1か所）。id を追加・削除・改名したら `doc-sync` サブエージェントで md 全体の言及を同期する。

### 6.3 人が目視する — [`TESTING.md`](TESTING.md) §3.1 の手動スモーク

機械にできないもの（レスポンシブ・ダーク・空状態・実操作・着脱耐性の DOM 層）は [`TESTING.md`](TESTING.md) §3.1 に1か所へ集約した。**DoD の人間側はそのリストが正**（本書では再掲しない）。手動チェックの器は §3「レイヤ別の担保方法」に既にあり、新しい md を増やすより既存の器に吸収するほうが参照先が散らばらないため、そこへ置いた。
