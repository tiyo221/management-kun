...
1. **主操作は1アクション** — 各モジュールには「一番多く繰り返す操作」が1つある（todo＝Inbox の仕分け、daily＝時間の割当、skills＝評価の入力）。これを個別仕様 [`spec/modules/<id>.md`](spec/modules/) に `## 主操作` として明記し、**1アクションで完了させる**。主操作にモーダルを挟まない。**節の有無は機械が見る**（§6.1。導入時点で欠けていた既存分だけ `test/spec-consistency.test.js` の `MAIN_ACTION_PENDING` に明示して外してある ── 埋めたらその一覧からも消す）。
...
- **アクセシビリティ・キーボード**（[`spec.md`](spec.md) §10.2） — 主要操作がキーボードで到達可能／記号だけのコントロールに読み上げ名（`aria-label` / `aria-labelledby`。`title` は読み上げ名にならない）。実際に触って確かめる部分は §6.3。
