...
- [ ] `summary()` の `empty` がデータ皆無のときだけ `true`。ここが誤ると、空のモジュールに出る「サンプルを入れて試す」バー（[`spec.md`](spec.md) §3.6.2）が実データのある画面に出て、全置換の `loadSample()` でデータを失う。空の状態でモジュールを開いてバーが出ること・投入後に「サンプルを片付ける」で戻ることを確認する。
