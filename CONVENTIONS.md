...
- **狙い 2 の判定材料は2点だけ**: (1) DOM に触れない（下記 §1.3）、(2) 振る舞いがテストで期待値として固定されている。狙い 2 を満たすかはこの2点だけで判定する、という意味であって、**§1.3 のその他の規則（`MK.store` 経由のみ・`render` を呼ばない・JSDoc 等）や §1.4 は別途遵守する**。うち JSDoc のように**狙い 2 に資する**ものもある（取り出す側は JSDoc を規則の記述として読む）が、**資することと判定材料であることは別**で、充足判定は上の2点だけで行う。DOM から切れているから node で叩け、node で叩けるからテストの期待値が別 PJ 側の受入条件として使える。**テスト可能性は狙いではなく手段**。
