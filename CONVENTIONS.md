...
サーバー版は**別プロジェクトとして進行中**で、本リポジトリの `logic.js` をそのまま載せ替える形は取らない。したがって狙いは「このコードをサーバーへ移送できること」ではなく、**ドメイン規則が view の中に埋もれず、独立して取り出せること**を指す。

- **満たすべきは2点だけ**: (1) DOM に触れない（下記 1.3）、(2) 振る舞いがテストで期待値として固定されている。DOM から切れているから node で叩け、node で叩けるからテストの期待値が別 PJ 側の受入条件として使える。**テスト可能性は狙いではなく手段**。
