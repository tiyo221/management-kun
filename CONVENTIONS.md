...
2. **ドメイン規則を view から独立させ、別プロジェクトへ取り出せる形に保つ**（下記 1.1.1）。

> 共有層も同じ分離になっている（`store.js`/`io.js`/`people.js`/`projects.js` = ロジック、`ui.js`/`design.css` = 描画）。

#### 1.1.1 狙い 2 が意味すること（取り出せること ≠ 移送できること）
サーバー版は**別プロジェクトとして進行中**で、本リポジトリの `logic.js` をそのまま載せ替える形は取らない（#310）。したがって狙いは「このコードをサーバーへ移送できること」ではなく、**ドメイン規則が view の中に埋もれず、独立して取り出せること**を指す。

- **狙い 2 の判定材料は2点だけ**: (1) DOM に触れない（下記 1.3）、(2) 振る舞いがテストで期待値として固定されている。狙い 2 を満たすかはこの2点だけで判定する、という意味であって、**§1.3 のその他の規則（`MK.store` 経由のみ・`render` を呼ばない・JSDoc 等）や §1.4 は別途遵守する**（それらは狙い 1・§1.4 の要件）。DOM から切れているから node で叩け、node で叩けるからテストの期待値が別 PJ 側の受入条件として使える。**テスト可能性は狙いではなく手段**。
...
- logic のデータアクセスは `store` 抽象の背後に閉じる。読み書き先が localStorage であることを logic の各所へ散らさず、`store` を差し替えれば view を変えずに保存先を変えられる状態を保つ（本リポジトリ自体のサーバー化は `spec.md` §13 のとおりスコープ外。§1.1.1）。
...
- [ ] `exportData`/`importData`（置換・マージ）・`loadSample` を実装。該当すれば旧データ移行の対応あり。scoped モジュールは3つとも対象 id を末尾の任意引数で受け取る（[`spec.md`](spec.md) §3.5）。
- [ ] `exportData()` が**自分の namespace のデータだけ**を返し、空のとき「スカラ＋空の入れ物」になる。空のモジュールに出る「サンプルを入れて試す」バー（[`spec.md`](spec.md) §3.6.2）はこれで空を判定するため、空でも中身のある値を返すとバーが出ない。空の状態でモジュールを開いてバーが出ること・投入後に「サンプルを片付ける」で戻ることを確認する。
...
- [ ] モジュールを追加・削除・改名したら、その id で Git 追跡下の md 全体（`git ls-files "*.md"`。`.claude/agents/*.md` も含む）を grep し、§5 以外に残る言及を同期した（読んだ箇所だけでなく `grep <id>` で探しに行く。網羅に見える列挙は「例:」の少数へ刈る。[`CLAUDE.md`](CLAUDE.md) 必須ルール5・#227）。
