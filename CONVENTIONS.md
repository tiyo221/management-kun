...
2. **編集はインライン既定** — 一覧項目の編集はその場で行う（クリックで入力欄化、Enter 確定・Esc 取消）。モーダルを使ってよいのは「3項目以上を同時に変える」「新規作成で必須項目が複数ある」ときだけ。共通ヘルパを使い、各モジュールで自作しない（ヘルパ本体は最初の消費者と同時に `shared/ui.js` へ入れる。#244 の todo が1件目。[`CODING.md`](CODING.md)「共通化は2か所以上で必要になってから」）。付随して次を守る:
   ...
   - **行が握る項目は描画時のスナップショット**（`load()` が毎回 store から作り直すので live 参照ではない）。インライン編集で確定したら**その場のオブジェクトにも書き戻す** ── しないと、直後の削除トーストやモーダルの初期値が旧い値になる。
...
| 余白のインライン直書きが無い（§2.1） | 〃 |
| **モジュールの**色がトークン経由で直書きが無い（§2.1。走査対象は `modules/*` で `shared/*` は含まない。ダークで実際に見るのは §6.3） | 〃 |
...
- **§2.1 余白・リズム／トークン** — 隣接ブロックが密着しない（`.card` 直下・末尾アクションの落とし穴に注意）。色・余白・角丸・タイポは [`DESIGN.md`](DESIGN.md) のトークン経由で、値を直書きしない（このうち機械が見るのは §6.1 のとおり色と余白だけ。**角丸・タイポは人が見る**）。
