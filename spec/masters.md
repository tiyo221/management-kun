...
- CSV 入出力に対応（§4.6）。列: `プロダクト名, ステータス, 責任者, 概要, リポジトリ, タグ`。`ステータス` は key（`planned` 等）または日本語ラベル先頭語（計画 / 稼働 / 保守 / 終息）を受け付け、不明なら `planned`。`タグ` は空白 / カンマ区切り。**現行は全置換**だが、共通契約（§4.4.1 C）に合わせ名寄せ upsert へ寄せる。
