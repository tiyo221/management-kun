...
## レイアウト
統合アプリの幅（~950px）では両方を同時に置くと窮屈なため、**「テーブル / ガント」タブで片方を全幅表示**する（Issue #157）。ガントのバーにタスク名を載せ、単独でも読めるようにする。
...
- **今日へスクロール**: ガントのツールバーのボタンで今日の位置を中央に寄せる。ガントを開いた／ズームを変えた直後も今日付近を初期表示にする。
