...
技術の削除は確認を出さず即実行し、`MK.ui.undoDeleteToast` で取り消せる（CONVENTIONS §2.5-3）。`removeItem` は**削除できたかを返す**（その id が無ければ `false`）ので、view は空振りではトーストを出さない。
...
## サマリー（HOME 表示）
`summary(today?)` は行動につながる指標として `保留（Hold）M件`（判断待ち）／`評価中 K件`（Assess＋Trial ＝採否の判断が要る）を返す純関数（[`spec.md`](../../spec.md) §3.6）。母数（技術総数）と、`attention` の見直し期限と丸かぶりする期限 stat は撤去した（方針①③・Issue #206）。`attention` として見直し期限の超過（error）／90日以内（warn）を申告する（HOME の要対応帯・Issue #102）。

## 任意契約の採否（searchItems / summaryFor）
グローバル検索（[`spec.md`](../../spec.md) §3.5）と人／PJ 詳細の集約（§3.6.1）は任意契約。採否を固着させる（#220）。
- **searchItems**: 実装。全アイテムを `label`＝名前・`sub`＝採用状況+カテゴリ・`keywords`＝バージョン/メモ/タグで供給する。
- **summaryFor**: 見送り（保留）。技術は product エンティティに紐づくが summaryFor は人／PJ 対応が前提で、product 拡張は必要になった時点の後続 Issue に回す（#220 スコープ外）。
