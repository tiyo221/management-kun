...
## 主操作
**リング（採用状況）を動かす**（CONVENTIONS §2.5-1）。一覧の各行に置いたリング select を1アクション（開いて選ぶ）で切り替えて完了する。モーダルを挟まない。台帳としての本題は「いま何を標準採用し、何を撤去中か」を最新に保つことなので、Adopt / Trial / Assess / Hold を行き来させる操作が最も多く繰り返される。付随して、技術名は行内で直接編集し（インライン・§2.5-2）、カテゴリ・バージョン・メモ・見直し期限・タグは「編集」モーダルでまとめて編集する。リングを変えた行が絞り込み中のタブから外れたら、その行だけ取り除いて件数バッジを更新する（全再描画しない・§2.5-4）。
