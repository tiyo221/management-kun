...
## 役割
マネージャの3つの問いに月次で答える **PJ 横断の cross ビュー**（[`spec.md`](../../spec.md) §3.7.5）。**People を主語に Project 次元を横断集約**し、アロケーション（planning）を編集する。時間軸は **月次×長ホライズン**（週次は「見えても確保が間に合わない」ため見せない）。**主単位は人（FTE、1人＝100%）**とし、`必要 2.0人 / 確保 1.1人 / あと 0.9人不足` のように表す（% は補助表記）。
...
4. **計画の入力** — アロケーション（供給）と需要のレコードを追加・編集・削除する（判断のもとになるデータ）。
