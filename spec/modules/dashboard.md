...
**最上段の基準（§2.5-5）は各カードの遷移導線「〜を開く →」（`ctx.route`）とする。** 画面で最も多い操作はこの遷移で、1クリックで完了しモーダルを挟まない。主操作が無いことは「頻度順を問わなくてよい」を意味しないので、カードの並び・最上段に置く要素はこの遷移を基準に決める。将来この画面へ編集を持ち込む案が出たときは、まず「編集は各モジュール側」（読み取り専用の集約ビューという位置づけ）を崩すかどうかを決めるのが先で、主操作の宣言はそのあとに定める。
