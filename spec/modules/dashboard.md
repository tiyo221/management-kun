...
## 主操作
**持たない（読むためのモジュール）**（CONVENTIONS §2.5-1 が「持たない」を認める条件＝自前の永続データを持たない読み取り専用ビュー、に当たる）。dashboard は自前の永続データを持たず（下記「固有データ」）、編集はすべて他所（各モジュールまたはマスタ）で行う（上記「役割」）ため、更新にあたる操作がこの画面に存在しない。**主操作が無くても節は立てる** ── 節ごと省くと、次に触った人が「主操作が無いモジュール」と「書き忘れ」を区別できない。
