...
**持たない（読むためのモジュール）**（CONVENTIONS §2.5-1）。dashboard は自前の永続データを持たず（下記「固有データ」）、編集はすべて各カードの遷移導線から該当モジュール側で行う（上記「役割」）ため、「一番多く繰り返す操作」に当たる更新操作がこの画面に存在しない。**主操作が無くても節は立てる** ── 節ごと省くと、次に触った人が「主操作が無いモジュール」と「書き忘れ」を区別できない。

画面で最も多い操作は各カードの「〜を開く →」（`ctx.route`）で、これは1クリックで完了しモーダルを挟まない。将来この画面へ編集を持ち込む案が出たときは、まず「編集は各モジュール側」（読み取り専用の集約ビューという位置づけ）を崩すかどうかを決めるのが先で、主操作の宣言はそのあとに定める。
