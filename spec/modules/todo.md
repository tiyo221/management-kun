...
## 主操作
**Inbox の仕分け（タスクを次のステータスへ送る）**（CONVENTIONS §2.5-1）。一覧の各行に置いたステータス select を1アクション（開いて選ぶ）で切り替えて完了する。モーダルを挟まない。捕捉（Enter で Inbox へ追加）→仕分け→整理、が日々の中心の流れであり、Inbox から他ステータスへ送る操作が最も多く繰り返される。**捕捉した項目は投入先が Inbox 固定なので、追加後は Inbox タブ＋検索なしへ絞り込みを寄せる**（§2.5-2。寄せないと別タブや検索中の捕捉が一覧に現れず、バッジの数字だけ増える）。付随して、タイトル・期日は行内で直接編集し（インライン／`type=date`）、メモ・コンテキスト・プロジェクトの3項目だけを「詳細」モーダルでまとめて編集する。削除は確認を出さず即実行し `MK.ui.undoDeleteToast` で取り消せる（§2.5-3）。
...
- 初期タブは **Inbox**（GTD 思想＝Inbox Zero を後押し・追加直後のタスクが消えない・Issue #164）。「全て」タブは残し従来通り全件を見られる。
