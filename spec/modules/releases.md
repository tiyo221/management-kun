...
リリースの削除は確認を出さず即実行し、`MK.ui.undoDeleteToast` で取り消せる（CONVENTIONS §2.5-3）。`removeRelease` は**削除できたかを返す**（その id が無ければ `false`）ので、view は空振りではトーストを出さない。`updateRelease` も更新できたかが分かる契約（その id が無ければ `null`）で、行内の編集口はこの戻り値を見て、消えた行への操作を受け入れずに画面をストアへ合わせ直す。

## 主操作
**リリースのステータスを進める**（CONVENTIONS §2.5-1）。一覧の各行に置いたステータス select を1アクション（開いて選ぶ）で予定（Planned）→ 完了（Done）へ切り替える。モーダルを挟まない。台帳としての本題は「何をいつ出したか／いつ出すか」を最新に保つことなので、運用中はこの更新が最も多く繰り返される。付随して、バージョン / 名称は行内で直接編集し（インライン・§2.5-2）、プロダクト・予定日・実施日・メモは「編集」モーダルでまとめて編集する。ステータスを変えた行が絞り込み中のタブから外れたら、その行だけ取り除いて件数バッジを更新する（全再描画しない・§2.5-4）。

**完了（Done）にしても実施日（`actualDate`）は自動で打刻しない**（todo の `completedAt` と非対称だが意図的）。`actualDate` は表示日（`effectiveDate`）の一次ソースで、打刻すると行の並びがその場で動く ── 主操作は「送って終わり」であってほしいのに、送った行が目の前で別の位置へ跳ぶ。加えてリリース日は「操作した日」とは限らない（後追いで台帳を埋める運用がある）ので、当日を既定で入れると事実と違う日付が静かに入る。実施日はモーダルで入れる。

追加・編集の保存で**その1件が絞り込みから外れるときは、絞り込みをその1件へ寄せてから再描画する**（§2.5-2）。releases は投入先がモーダルで選べるので、寄せ先は固定値ではなく保存した `productId` / `status` になる ── 寄せないと、「予定」タブを開いたまま完了で追加した行が、保存できているのに一覧から消える。
