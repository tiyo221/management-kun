...
- **完了は双方向で揃う（todo が正）**:
  ...
  - **スナップショットの治癒**: 読み取り時の解決は表示を揃えるだけで保存値は古いまま残る。そのため `rolloverTo`（夕方の締め＝書き込み経路）で解決値を `done` へ書き戻す。これを怠ると、todo 側で完了 → その後 todo を削除、の順で古い `false` をフォールバックが拾い、完了済み項目が未完了として復活して毎日繰り越され始める。
