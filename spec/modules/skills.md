...
スキルの削除は確認を出さず即実行し、`MK.ui.undoDeleteToast` で取り消せる（CONVENTIONS §2.5-3）。**削除の退避には紐づく評価（`ratings`）も含める** ── スキルだけ戻して評価が空だと、消したときより悪い状態（全メンバー分の入力し直し）になるため。`removeSkill` は**削除できたかを返す**（その id が無ければ `false`）ので、view は空振りではトーストを出さない。
