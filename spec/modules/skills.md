...
スキルの削除は確認を出さず即実行し、`MK.ui.undoDeleteToast` で取り消せる（CONVENTIONS §2.5-3）。**削除の退避には紐づく評価（`ratings`）も含める** ── スキルだけ戻して評価が空だと、消したときより悪い状態（全メンバー分の入力し直し）になるため。`removeSkill` は**削除できたかを返す**（その id が無ければ `false`）ので、view は空振りではトーストを出さない。`updateSkill` も同じ契約で、**その id が無ければ `false` を返して保存もしない**（空振りで保存すると、直前の削除の退避＝取り消しを潰してしまう）。行内の編集口（表示・コア・スキル項目名）はこの戻り値を見て、消えた行への操作を受け入れずに画面をストアへ合わせ直す。

## 主操作
**メンバー×スキルにレベルを付ける**（CONVENTIONS §2.5-1）。「紐づけ（評価入力）」タブのマトリクスで、セルの 1〜5／対象外セグメントを1クリックして即確定する（同じ数字の再クリックで解除）。モーダルを挟まず、押したセルだけ描き直す。台帳としての本題は「誰が何をどこまでできるか」を最新に保つことで、人数×スキル数だけ繰り返されるこの操作が最も多い。付随して、スキル管理タブではスキル項目名（中分類）を行内で直接編集し（インライン・§2.5-2）、「表示」「コア」は行内チェックボックスで切り替える。大分類・小分類・目標レベル・必要人数はコア設定と絡んで意味が決まるため「編集」モーダルでまとめて編集する。
