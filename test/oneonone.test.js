...
test("oneonone: summary は未完アクションと要フォロー・期限切れアクションを返す（累計 記録数 は撤去）", (MK) => {
  // 観点: 行動指標へ差し替え（累計 記録数 を撤去・stats↔attention の同一事実重複なし・#203）。基準日は引数で固定
  // 入力: 基準日 2026-08-05。m1=最終7/01（35日＝30日超で要フォロー）＋期限切れ未完アクション1・期限内未完1、m2=最終7/20（16日＝30日以内）
  // 期待: 空は empty=true。stats[0]=未完アクション2・stats[1]=要フォロー1、attention に 期限切れアクション1件（warn）
  const O = MK.logic.oneonone;
  const base = "2026-08-05";
  const s0 = O.summary(base);
  eq(s0.empty, true);
  eq(s0.stats[1].value, 0);      // 要フォローなし
  eq(s0.attention.length, 0);    // 期限切れなし
  O.addEntry({ memberId: "m1", date: "2026-07-01", body: "b", actions: [
    { text: "遅延", due: "2026-07-10" },   // 期限切れ（未完）
    { text: "余裕", due: "2026-08-10" },   // 期限内（未完）
  ] });
  O.addEntry({ memberId: "m2", date: "2026-07-20", body: "b2" });
  const s = O.summary(base);
  eq(s.empty, false);
  eq(s.stats.length, 2);                       // 累計 記録数 は撤去
  eq(s.stats[0].label, "未完アクション"); eq(s.stats[0].value, 2);
  eq(s.stats[1].label, "要フォロー"); eq(s.stats[1].value, 1);  // m1 のみ30日超
  eq(s.attention.length, 1);
  eq(s.attention[0].label, "期限切れアクション 1件");
