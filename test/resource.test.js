...
test("resource: 外注判定は単一の空きプールから消費する＝器をまたいだロール空とロール指定で空きを二重計上しない（問い②・#134）", (MK) => {
  // 観点: ロール指定の不足はそのロールのメンバーの空きでしか賄えない＝先に自ロールの空きを消費し、役割を問わない
  //       不足（ロール空）は残余（誰でも良い枠）からのみ吸収する。同じメンバーの空きを両バケットで二重に使わない。
  // 入力: 器A エンジニア需要60（供給0）、器B ロール空需要70（供給0）。メンバーは m1(エンジニア) が別PJに40%（空き60）、
  //       m2(デザイナー) は別PJに100%（空き0）＝チーム総空き60。エンジニア枠が60を使い切ると誰でも良い枠は0
  // 期待: エンジニア不足60は自ロール空き60で吸収→外注0。残余0のため器B の70はまるごと外注（旧実装は総空き60を
  //       再カウントし70-60=10 とだけ判定して50を取りこぼす）。outsource=70 / shortage=130
  const S = MK.logic.resource;
  const members = [{ id: "m1", role: "エンジニア" }, { id: "m2", role: "デザイナー" }];
  const allocations = [
    { memberId: "m1", targetId: "c", percent: 40, startDate: "2026-07-01", endDate: "2026-07-31" },
    { memberId: "m2", targetId: "c", percent: 100, startDate: "2026-07-01", endDate: "2026-07-31" },
  ];
  const demands = [
    { targetId: "a", role: "エンジニア", requiredPercent: 60, startDate: "2026-07-01", endDate: "2026-07-31" },
    { targetId: "b", role: "", requiredPercent: 70, startDate: "2026-07-01", endDate: "2026-07-31" },
  ];
  const targets = [{ id: "a", name: "PJ-A", dim: "project" }, { id: "b", name: "PJ-B", dim: "project" }, { id: "c", name: "PJ-C", dim: "project" }];
  const rows = S.outsourcingByMonth(allocations, demands, targets, members, ["2026-07-01"]);
  eq(rows[0].shortage, 130);      // 60 + 70
  eq(rows[0].internalFree, 60);   // 総空き（m1 の 60 のみ）
  eq(rows[0].outsource, 70);      // エンジニアで60使い切り→器Bの70は残余0でまるごと外注
  eq(rows[0].needsOutsource, true);
});
