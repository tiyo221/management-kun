...
test("questions: summary は未解決件数を stats に出す（throughput 系は撤去・#202）", (MK) => {
  // 観点: summary.stats は未解決（バックログ）のみ。今週わかった等の throughput 系は出さない
  // 入力: 2件追加し、1件を今日 resolved にする
  // 期待: empty=false、stats は1要素で stats[0].value=1（未解決）。resolvedThisWeek は集計自体は残す
  const Q = MK.logic.questions;
  Q.addItem("未解決の項目");
  Q.addItem("今週わかる項目");
