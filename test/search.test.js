...
test("oneonone.searchItems: 本文・人名・アクションを keywords に載せ、空エントリは除外", (MK) => {
  // 観点: 1on1 メモを本文・相手の人名で引けるようにする（#220）。本文もアクションも無い空エントリは除外
  // 入力: 田中との「キャリアの相談」(action="スキルマップ整理") と、本文もアクションも空のエントリ
  // 期待: 中身のあるエントリを含み空は除外。keywords に本文・人名・アクション本文、sub に本文冒頭が載る
  const O = MK.logic.oneonone;
  const tanaka = MK.people.resolveOrCreate("田中");
  O.addEntry({ memberId: tanaka, date: "2026-07-10", body: "キャリアの相談を受けた", actions: [{ text: "スキルマップ整理" }] });
  O.addEntry({ memberId: tanaka, date: "2026-07-11", body: "", actions: [] });
  const rows = O.searchItems();
  eq(rows.length, 1, "空エントリを除外");
  const r = rows[0];
  assert(r.keywords.indexOf("キャリアの相談を受けた") >= 0, "本文を keywords に含む");
  assert(r.keywords.indexOf("田中") >= 0, "相手の人名を keywords に含む");
  assert(r.keywords.indexOf("スキルマップ整理") >= 0, "アクション本文を keywords に含む");
  assert(/田中/.test(r.label), "label に相手の人名");
  assert(/キャリア/.test(r.sub), "sub に本文冒頭");
});
