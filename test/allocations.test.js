/* allocations（共有アロケーションマスタ）— CRUD・集計純関数・退役 workload からの吸い上げ移行（Issue #45 / #167） */
"use strict";
...
test("allocations: percentOn は期間内の全器割当を合算する純関数", (MK) => {
  // 観点: 「空き要員 = 総キャパ − 全器へのアロケーション合計」の集計元となる純関数（resource が再利用）
...
test("allocations: migrateFromWorkload は退役 workload 内部のアロケーションを移設しキーごと破棄する（加算的・冪等）", (MK) => {
  // 観点: Issue #45 の昇格＋#167 の退役。旧 workload 内部データを共有マスタへ移し、workload 名前空間は破棄する
  // 入力: workload 名前空間に allocations を含む旧形状を直接書き込む
  // 期待: 移設後は共有マスタに2件、workload キーは消え、再実行は0件（冪等）
  const A = MK.allocations;
  ...
  eq(MK.store.read("module:workload"), null); // 退役 workload 名前空間はキーごと破棄される
  assert(localStorage.getItem(MK.store.keyOf("module:workload")) == null, "localStorage からも消える");
...
test("allocations: migrateFromWorkload は既存 id を上書きしない", (MK) => {
  ...
  // 期待: マスタの wa1 は 10 のまま、移設件数は0、workload キーは破棄される
  const A = MK.allocations;
  ...
  eq(MK.store.read("module:workload"), null); // アロケーション有無に関わらずキーを破棄
});
