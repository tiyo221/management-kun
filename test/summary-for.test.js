...
test("summaryFor: skills は他人の評価に引きずられない／未対応種別は empty", (MK) => {
  ...
  // 入力: 「自分」に評価なし・「他人」に評価5を付けた状態で summaryFor("person", 自分)／summaryFor("project", 自分)
  // 期待: 自分は他人の評価に引きずられず empty、person 以外の種別は empty で stats=[]
  const S = MK.logic.skills;
...
test("summaryFor: resource の未対応種別は empty", (MK) => {
  ...
  // 入力: summaryFor("project", "pj1") を呼ぶ
  // 期待: empty=true・stats=[]（person 以外の種別は担当外）
  const R = MK.logic.resource;
...
test("summaryFor: oneonone の未対応種別は empty、記録ゼロの最終実施は '-'", (MK) => {
  // 観点: 記録ゼロでも安全な既定値を返し、person 以外の種別は担当外として empty
  // 入力: 記録が1件も無いメンバーに summaryFor("person", p)／summaryFor("product", p)
  // 期待: person は empty=true で最終実施 stats[1]="-"、product 種別は empty
  const O = MK.logic.oneonone;
