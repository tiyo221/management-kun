...
// ---- workload ----
test("summaryFor: workload はタスクなしは empty、投入後は担当タスク数・平均稼働・状態", (MK) => {
  // 観点: 人の稼働概況（担当タスク数・直近4週平均稼働率%・状態ラベル）を集約する
  // 入力: 今週（月〜日）に負担40のタスク1件を担当者へ
  // 期待: 空は empty=true / 追加後 empty=false・担当1・平均稼働は"…%"・状態は既知ラベル
  const W = MK.logic.workload;
  const p = MK.people.resolveOrCreate("稼働太郎");
  assert(W.summaryFor("person", p).empty, "タスクなしは empty");
  assertShape(W.summaryFor("person", p));
  const mon = MK.util.mondayOf(MK.util.todayISO());
  W.addTask({ memberId: p, title: "x", load: 40, startDate: mon, endDate: MK.util.addDays(mon, 6), status: "in_progress" });
  const s = W.summaryFor("person", p);
  assert(!s.empty);
  eq(s.stats[0].value, 1, "担当タスク数");
  assert(/%$/.test(String(s.stats[1].value)), "平均稼働は % 表記");
  assert(["過負荷", "適正", "余裕"].indexOf(s.stats[2].value) >= 0, "状態ラベル");
  assert(W.summaryFor("product", p).empty, "person 以外の種別は empty");
});
