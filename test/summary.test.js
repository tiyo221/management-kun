...
test("summary: wbs は葉タスクが無ければ empty、PJ 軸の件数（進行中PJ/未着手PJ）を出す（混合平均は撤去）", (MK) => {
  // 観点: wbs の summary は葉タスクが無ければ empty（親のみでは非空にしない）。stats は PJ 軸の件数で
  //       混合平均の進捗% と合計 進行中 は出さない（#204 方針③）
  // 入力: 空を確認 → addRoot()（葉1・未着手）
  // 期待: 空は empty=true / 追加後 empty=false・進行中PJ=0・未着手PJ=1（葉が全て未着手）。進捗/合計 stat は無い
  ...
  const s = W.summary();
  assert(!s.empty);
  eq(s.stats.length, 2, "stats は2件（混合平均・合計は撤去）");
  eq(s.stats[0].label, "進行中PJ"); eq(s.stats[0].value, 0);
  eq(s.stats[1].label, "未着手PJ"); eq(s.stats[1].value, 1);
  s.stats.forEach((x) => assert(x.label !== "進行中" && x.label !== "進捗", "合計 進行中・混合平均 進捗 は出さない: " + x.label));
});
...
test("attention: wbs は期限超過を代表 PJ 名＋件数で error 申告する（未完かつ終了日<基準日・全PJ横断）", (MK) => {
  // 観点: 期限超過（isOverdue: 未完かつ終了日<基準日）を全PJ横断で数え attention(error) に昇格する。単一 PJ（従来の
  //       単一 store）では代表 PJ（無名）＋件数のラベルにする（#204）
...
test("summary/attention: wbs は PJ 軸で件数化し、代表 PJ を名指しで error 申告する（全PJ横断・#204）", (MK) => {
  // 観点: PJ が複数あるとき summary は主語を PJ へ寄せて件数化する。stats=進行中PJ/未着手PJ、
  //       attention=最も遅れている代表 PJ を名指し＋「ほかN件」に畳む（PJ 数に依存せず一定の情報量）
  // 入力: project 次元で PJ-A（進行中1・期限超過2）／PJ-B（完了1＋期限超過の未着手1）／PJ-C（葉が全て未着手・期限内）
  // 期待: 進行中PJ=1（A のみ）・未着手PJ=1（C のみ。B は完了葉があり全未着手ではない）。
  //       attention は代表 PJ-A（超過最多=2）＋「ほか1件」（残る B）
  ...
  try {
    ...
    const c = MK.projects.create({ name: "PJ-C" });
    W.importData({ version: 1, uid: 4, tasks: [
      { id: 1, level: 0, name: "A進行中", status: "inprogress", end: "2099-01-01", deps: [] },
      { id: 2, level: 0, name: "A遅延1", status: "inprogress", end: "2026-07-01", deps: [] },
      { id: 3, level: 0, name: "A遅延2", status: "notstarted", end: "2026-07-02", deps: [] },
    ] }, "replace", a.id);
    W.importData({ version: 1, uid: 3, tasks: [
      { id: 1, level: 0, name: "B完了", status: "done", end: "2026-07-01", deps: [] },
      { id: 2, level: 0, name: "B遅延", status: "notstarted", end: "2026-07-01", deps: [] },
    ] }, "replace", b.id);
    W.importData({ version: 1, uid: 2, tasks: [
      { id: 1, level: 0, name: "C未着手", status: "notstarted", end: "2099-01-01", deps: [] },
    ] }, "replace", c.id);
    const s = W.summary("2026-07-12");
    eq(s.stats[0].label, "進行中PJ"); eq(s.stats[0].value, 1, "進行中タスクのある PJ は A のみ");
    eq(s.stats[1].label, "未着手PJ"); eq(s.stats[1].value, 1, "葉が全て未着手の PJ は C のみ（B は完了葉があり全未着手ではない）");
    assert(Array.isArray(s.attention) && s.attention.length === 1, "attention 1件");
    eq(s.attention[0].severity, "error");
    assert(s.attention[0].label.includes("PJ-A"), "代表は超過最多の PJ-A: " + s.attention[0].label);
    assert(s.attention[0].label.includes("ほか1件"), "残る1 PJ（B）を件数に畳む: " + s.attention[0].label);
