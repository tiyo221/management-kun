...
test("store: usage() は mk: プレフィックスキーの合計バイト・比率を返す", (MK) => {
  ...
  // 入力: mk: な settings を1件書き、非 mk: な other:x も直接セットして usage()
  // 期待: count=1（other:x 除外）、quota=5MB、ratio=bytes/quota、bytes=(キー長+値長)*2 と一致
...
test("store: 書込成功で lastWriteError は null、戻り値 true", (MK) => {
  // 観点: 正常な書込では成功を返し、直近エラー状態を残さない
  // 入力: settings に {a:1} を write
  // 期待: 戻り値 true、lastWriteError は null
...
test("store: QuotaExceededError を握りつぶさず記録し、戻り値 false・キャッシュは新値を保持", (MK) => {
  // 観点: 容量超過を黙って捨てず記録し、_cache には新値を残して JSON 退避で救えるようにする
  // 入力: setItem が必ず QuotaExceededError を投げる localStorage スタブに差し替え、settings を write
  // 期待: 戻り値 false、lastWriteError に quota:true/ns:"settings" が記録、read は新値 {big:"x"} を返す
...
test("store: remove() は localStorage キーとキャッシュを落とし、例外環境でも握りつぶす", (MK) => {
  ...
  // 入力: module:todo を書き read でキャッシュ後 remove。次に removeItem が投げるスタブで module:goals を remove
  // 期待: 通常は localStorage キーと _cache 両方が消え read=null。例外環境でも throw せず _cache からは落ちる
...
test("store: collection() load は未保存・不正形式で既定 { version, [key]: [] } を返す", (MK) => {
  ...
  // 入力: collection("module:todo",{key:"tasks"}) を未保存で load、次に tasks="not-array" を書いて load
  // 期待: どちらも既定 { version:1, tasks:[] } を返し、既定オブジェクトは毎回新規インスタンス
...
test("store: collection() load は保存済みデータをそのまま返す", (MK) => {
  // 観点: 妥当な保存データは既定で上書きせず、そのまま読み戻す
  // 入力: tasks 配列と exportedAt を含む正常データを write してから collection.load()
  // 期待: 保存したオブジェクトと完全一致で返る
...
test("store: collection() save は stamp:true で exportedAt を付与し、保存成否を返す", (MK) => {
  ...
  // 入力: stamp:true の collection と stamp 未指定の collection それぞれで save
  // 期待: stamp:true は exportedAt を付与し保存にも反映・戻り値 true、未指定は exportedAt を付けない
...
test("store: clearAll() は mk: キーのみ削除し、非 mk: と _cache を初期化する（Issue #176）", (MK) => {
  // 観点: 全データ初期化。mk: プレフィックスのキー（既知 namespace＋scoped 対象別キー）を消し、
  //       非 mk: キー（他ツール・旧ツール）は残す。_cache も空になり read は null を返す。
  // 入力: mk: な people と scoped な module:wbs:pj1 を書き、非 mk: の other:x と旧ツールキーもセットして clearAll
  // 期待: removed=2（mk: の2件のみ）、mk: キーと _cache は消え read=null、非 mk:/旧ツールキーは残る
  MK.store.write("people", { version: 1, list: [{ id: "m1" }] });
  MK.store.write("module:wbs:pj1", { version: 1, tasks: [{ id: "t1" }] }); // scoped 対象別キー
  MK.store.read("module:wbs:pj1"); // _cache に載せる
  global.localStorage.setItem("other:x", "keep"); // 非 mk: は対象外
  global.localStorage.setItem("task-tool-data-v1", "legacy"); // 旧ツールキー（非 mk:）も残す

  const removed = MK.store.clearAll();
  eq(removed, 2, "mk: キー2件だけ削除する");
  eq(global.localStorage.getItem(MK.store.keyOf("people")), null, "既知 namespace キーが消える");
  eq(global.localStorage.getItem(MK.store.keyOf("module:wbs:pj1")), null, "scoped 対象別キーも消える");
  eq(global.localStorage.getItem("other:x"), "keep", "非 mk: は残す");
  eq(global.localStorage.getItem("task-tool-data-v1"), "legacy", "旧ツールキーは残す");
  eq(MK.store.read("people"), null, "_cache も初期化され read は null");
  assert(!("module:wbs:pj1" in MK.store._cache) || MK.store._cache["module:wbs:pj1"] === null, "scoped も _cache から落ちる");
});

test("store: clearAll() は削除不可な環境でも例外を伝播しない（Issue #176）", (MK) => {
  // 観点: removeItem が投げる環境でも clearAll は例外を伝播せず _cache は初期化する（起動を止めない）
  // 入力: mk: キーを1件返し removeItem が必ず throw する localStorage スタブで clearAll
  // 期待: throw せず removeItem は実際に呼ばれ（握りつぶし経路を通る）、removed=0・_cache は {}
  MK.store.write("people", { version: 1 });
  const orig = global.localStorage;
  // removeItem が投げるスタブ。allKeys() が回るよう length/key を明示的に持たせて（Object.assign は
  // 非列挙の length をコピーしないため）、throwing な removeItem が実際に呼ばれる経路を検証する。
  let removeCalls = 0;
  const throwing = {
    getItem: (k) => orig.getItem(k),
    key: () => MK.store.keyOf("people"),   // mk: キーを1件返す
    removeItem: () => { removeCalls++; throw new Error("no remove"); },
  };
  Object.defineProperty(throwing, "length", { get: () => 1 });
  global.localStorage = throwing;
  try {
    const removed = MK.store.clearAll(); // 例外を伝播しない
    assert(removeCalls > 0, "removeItem が実際に呼ばれる（握りつぶし経路を通る）");
    eq(removed, 0, "削除できないので removed は 0");
    eq(MK.store._cache, {}, "例外環境でも _cache は初期化される（Object.keys 空）");
  } finally {
    global.localStorage = orig;
  }
});
...
test("store: onWriteError フックが呼ばれる（案内導線の差し込み点）", (MK) => {
  // 観点: 書込失敗時に onWriteError フックへ通知する（UI 側で容量オーバー案内を出す差し込み点）
  // 入力: quota スタブに差し替え onWriteError を登録した状態で write
  // 期待: フックが呼ばれ、渡る info に quota:true が入る
  const orig = global.localStorage;
  const q = makeQuotaLS();
  global.localStorage = q;
  let got = null;
