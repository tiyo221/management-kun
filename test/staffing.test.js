/* staffing（要員計画）ロジック — 空き算出・アサイン集約の純関数（#27） */
"use strict";

test("staffing: 空き = キャパ − 全器割当合計（期間内のみ・範囲外は満空き）", (MK) => {
...
test("staffing: 過剰アサインは負の空きで表す（クランプしない）", (MK) => {
...
test("staffing: キャパ引数で空きの基準を変えられる", (MK) => {
...
test("staffing: セル割当は member×target×期間で合算する", (MK) => {
...
test("staffing: overviewOn が PJ×メンバー表と空きを一括算出", (MK) => {
...
test("staffing: 単一PJでも自明な1件として成立（縮退）", (MK) => {
...
test("staffing: freeSeries は期間軸で空きを算出する", (MK) => {
...
test("staffing: アロケーション皆無なら summary は empty", (MK) => {
