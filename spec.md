...
- 集計は logic 側の純関数に置き、テスト可能にする（`test/summary.test.js`）。上記の設計方針（#172）に沿った目標指標の例: todo=未完/期日未設定、goals=達成率/期限超過、skills=不足スキル/未評価、resource=空き要員/過負担、wbs=遅延PJ/進行中PJ（母数は方針①で `stats` から排除する）。「今日」に依存する集計は基準日を引数で受け取れるようにする（決定的テスト。TESTING §1）。
