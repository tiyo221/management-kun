...
- **プロダクトはゾーン（作業ツール）モジュールをまだ持たない**（中身が無いゾーンはナビに空グループを出さないため **`MK_CONFIG.zones` には載せない**。§1.5）。ただし成果物そのものの台帳＝**Product マスタ（📦 プロダクト・§4.4）を新設**し、シェルレベルの「マスタ」グループに人・プロジェクトと並べた（Issue #37）。このマスタは将来 §3.7 の Product スコープ次元のマスタを兼ねる（次元化＝`MK_CONFIG.dimensions` への追加は別 Issue）。テクノロジーは techstack（Issue #36）の追加でゾーンを新設した。
...
- **Product はマスタ追加時に実体化**する。Product マスタ（`products`・`mk:products`）を People / Project と同格の横断マスタとして先に追加した（Issue #37・§4.4）が、これは器（マスタ）の提供までで、`MK_CONFIG.dimensions` への Product 追加（次元としての配線）は別 Issue に分ける（後述ガードレール準拠）。次元化時は config に1行足すだけで `master: "products"` が既存の products マスタに解決する。
...
- People / Project は使わない（プロダクトは特定の人・PJ に属さない成果物情報）。Project との緩い紐付け（プロダクト⇄デリバリー）は将来検討。
- 名寄せ（§8）: `resolve(name)` / `resolveOrCreate(name)` を提供（将来の次元対象解決・CSV 取込の名前参照用）。
- CSV 入出力に対応（§4.6）。列: `プロダクト名, ステータス, 責任者, 概要, リポジトリ, タグ`。`ステータス` は key（`planned` 等）または日本語ラベル先頭語（計画 / 稼働 / 保守 / 終息）を受け付け、不明なら `planned`。`タグ` は空白 / カンマ区切り。取込は**全置換**（`プロダクト名` が空の行はスキップ）。
