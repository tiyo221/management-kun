...
- なお、モジュール間クロス機能（wbs → workload 負荷流し込み、WBS 担当合計 vs アロケーション照合）は当初 Phase 2 の柱2 に含めていたが、Phase 3 へ送った（§9.6）。
...
**モジュール間クロス機能**（Phase 2 の柱2 から移設）: wbs → workload の負荷流し込み、WBS 担当合計 vs アロケーション照合。いずれも複数モジュールを結合する横断機能であり、柱1の任意契約＋着脱耐性が固まってから着手する。実装は必ず任意契約＋欠損時グレースフルの上に載せる。
...
- （Phase 3）モジュール間クロス機能（例: wbs → workload 負荷流し込み、担当合計 vs アロケーション照合）。Phase 2 の柱2 から Phase 3 へ送った（§9.6）。
