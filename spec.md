...
| **プロダクト** | 作るもの（成果物） | —（ゾーンモジュールは将来／台帳は 📦 プロダクト マスタ・§4.4） | — |
...
- **プロダクトはゾーン（作業ツール）モジュールをまだ持たない**（中身が無いゾーンはナビに空グループを出さないため **`MK_CONFIG.zones` には載せない**。§1.5）。ただし成果物そのものの台帳＝**Product マスタ（📦 プロダクト・§4.4）を新設**し、シェルレベルの「マスタ」グループに人・プロジェクトと並べた（Issue #37）。このマスタは §3.7 の Product スコープ次元のマスタを兼ね、次元化（`MK_CONFIG.dimensions` への追加）は Issue #54 で完了済み（product-scoped モジュールはまだ無い＝ゾーンモジュール自体は将来）。テクノロジーは techstack（Issue #36）の追加でゾーンを新設した。
...
2. モジュール切替は**ゾーンに分けて表示**（§1.4）: 「**自分**」（ToDo / 目標 / わからないこと）｜「**ピープル**」（スキル / 負荷 / 要員計画 / 1on1）｜「**デリバリー**」（WBS）｜「**テクノロジー**」（技術スタック）。各ゾーンにラベルを付け、区切りで分離する（プロダクトは現状モジュールが無いため表示しない）。
3. メイン: 選択モジュールの描画領域。
