...
> この表は**ゾーンの分類（棚）・主語・People マスタ利用**という固有情報だけを示す。**各ゾーンに属するモジュールの正準一覧は §5・実際のゾーン割当は各エントリ HTML の `MK_CONFIG.zones`（§1.5）を正とする**。モジュール列をここに持たないのは、増減のたびに同期義務が生まれるのを避けるため（この表はモジュールを増減しても変わらない）。

- **領域名「デリバリー」**: プロジェクト遂行の領域だが、横断マスタの **Project**（§4.4）と語がぶつかるため、領域名には「プロジェクト」を用いず **デリバリー** と呼ぶ。
- **マスタ（人・プロジェクト・プロダクト）はゾーンに属さない**。各ゾーンのモジュールは People/Project マスタを**参照・編集**するが（「People マスタ」列＝そのゾーンがマスタを使うか）、マスタ自体は特定ゾーンの持ち物ではなく `home` / `settings` と同列の**シェルレベル管理カテゴリ**として独立させる（ナビでもゾーン群と設定の間の独立グループ「マスタ」に置く。§3.6 / §6.4）。とくに Project は wbs（デリバリー）だけでなく resource（ピープル）からも参照される横断マスタ（§4.4 / §4.6）であり、いずれかのゾーン配下に固定すると横断性が過小表現になる。
- **プロダクトゾーン**は releases（リリース台帳・横断）と metrics（プロダクト指標＝KGI/NSM/KPI・product-scoped）が担う。成果物そのものの台帳＝**Product マスタ（📦 プロダクト・§4.4）**はゾーンではなくシェルレベルの「マスタ」グループに人・プロジェクトと並び、§3.7 の Product スコープ次元のマスタを兼ねる（判断は [`spec/modules/releases.md`](spec/modules/releases.md) / [`spec/modules/metrics.md`](spec/modules/metrics.md)）。
- **自分ゾーン**はマネージャ個人の作業（目標・タスク・記録など。所属モジュールは §5）を扱う。中心に据える **goals は個人ツール**（自分の目標ロードマップ）であり、チームの KPI/OKR ではない。メンバーやプロジェクトと結び付けない（チーム目標管理に見せない）。
- **目標・指標系モジュールは領域ごとにネイティブ配置する（設計判断・確定）**。goals（自分＝個人目標）／チーム OKR・KPI（ピープル＝対象「メンバー・チーム」の正規住民）／プロダクト KGI・NSM・KPI（プロダクト＝product-scoped の metrics）は、それぞれの**対象領域の棚に独立して置く**。目標を横断レンズ化した単一モジュール＋スコープ切替や「目標／戦略」ゾーンの新設は**採らない**——本アプリは「EM の机（領域ごとの道具）」であり、「見る対象の領域で切る」本節の原則が上位だから（EM は対象を考えるときにその棚を開く）。3者は**互いに独立・低依存**とし相互リンクしない（連結が要れば Phase 3 のクロス機能）。UX 上の見た目の重複は許容し、机の独立性・統廃合しやすさを優先する。コード共有は先に共通エンジンを設計せず、骨格が実際に一致してから `shared/` へ抽出する（YAGNI・[`CODING.md`](CODING.md)）。プロダクト指標は metrics で実体化済み（#168）、チーム OKR/KPI は将来の別モジュール（§14）。
- **PJ 横断 / PJ 単位のスコープ軸**（例: 横断要員計画 vs PJ 単位 WBS）は、この領域分類とは**直交する別軸**。「同種のものを複数持つ／単一で持つ」を統一的に扱う **スコープ次元**として §3.7 に定義する（本節の分類には含めない）。

### 1.5 配布プロファイル（エントリHTML × `MK_CONFIG`）
1コードベースを維持したまま、**エントリHTMLごとに「積むゾーン／モジュール」を差し替える**ことで、用途別の配布形態を用意できる。フォーク（`shared/*` や todo/goals の二重化）はしない。現在のエントリは [`index.html`](index.html)（`profile: "manager"`・全部入り）1つで、宣言的なサブセット着脱の**機構は残す**（配布形態が必要になったらエントリ HTML を1つ追加するだけで済む）。

- **仕組み**: 各エントリHTMLが `window.MK_CONFIG = { profile, zones: [...] }` を宣言し、`shared/manifest.js`（構成マニフェスト・単一ソース・Issue #137）を1本読み込む。manifest が config に応じて共有資産＋必要なモジュールの `<script>`（logic→view）＋シェルを読込順を保って動的注入する（エントリ側で `<script>` を並べない）。`zones` を宣言しないエントリ（`index.html`）は manifest 既定（全部入り）を使い、`zones` を宣言したエントリはそのサブセットだけを積む。シェル（`shared/shell.js`）は config／manifest を読んで描画するだけで、どのプロファイルにも依存しない。エントリ・モジュールを足してもシェル本体の改修は不要。
- **分離の強さ**: 読み込まなかったモジュールは、コードもデータ（`localStorage`）も**物理的に存在しない**。見た目（ナビ非表示）だけでなく実データ的にも分離される。
- **サブセット配布の想定**: たとえばチームメンバーへ「自分ゾーン（todo / daily / goals / questions）のみ」の個人ツールを配る場合、`MK_CONFIG.zones` に自分ゾーンだけを宣言し `masters` を持たないエントリ HTML を1つ追加すれば、ピープル/デリバリー/プロダクト/テクノロジーの各モジュールとマスタ管理には到達不能なプロファイルになる。メンバーが入力したデータは既存の JSON エクスポート／インポート（§7）で回収できる（専用フローは作らない）。現状は配布実績が無いため、このエントリ HTML は持たない（機構だけを残す・#222）。理想形はビルド不要の制約下で「開示するモジュールだけを zip 等に詰める」配布パッケージ生成だが、必要になるまで作らない（§14）。
- 着脱耐性（外したモジュールがあっても起動・全画面・横断表示が壊れない）は §9.5・`test/module-detach.test.js` が担保する。
...
```
management-kun/
├── index.html              … シェル（マネージャ全部入りプロファイル）。これを開く。唯一のエントリ HTML。
│                             サブセット配布が必要になったら MK_CONFIG.zones を絞ったエントリを追加する（§1.5）
├── shared/
...
- マッチング（正規化・部分一致スコア・複数トークン AND・並べ替え）は DOM 非依存の純関数 `MK.search`（`shared/search.js`）に置き、テスト可能にする（`test/search.test.js`）。各モジュールは「いまアクセスしたい主要レコード」を返す（例: todo＝未完タスク、wbs＝全 PJ 横断の未完の葉タスク・Issue #144）。
...
- **HOME は1枚**。ゾーンごとに別 HOME は作らない。中身は HOME 固有の価値だけに絞り、**2段階**で描画する（Issue #100 / #169）: 先頭に「**要対応」帯**（表示中モジュールの `summary().attention` を集約した severity 別の色バッジ。Issue #102）、次に「ピン留め」セクション（`pinnedModules`・§4.8 のモジュールをサマリーカードのグリッドで表示）。**モジュール一覧の常設表示はサイドバー（常設ランチャー）に一本化し、HOME には置かない**（Issue #169。モジュール増加で HOME を肥大させない）。`MK_CONFIG.zones` を絞った配布用エントリ（§1.5）では、そのゾーンに載るモジュールだけが要対応帯・ピン留め候補に自動的に絞られる。
...
- `attention` は **stats 同様に任意**。未実装・不正形式でも HOME は壊さない（帯に出ないだけ）。各モジュールは「いま対応が要る事項」を severity 付きで返す（例: todo＝期限切れ/今日期限、resource＝過負荷（誰が過負荷か・warn）、wbs＝期限超過（代表 PJ＋件数・error）。#181 / #172）。
...
`summary()` が**モジュール全体**を主語にするのに対し、`summaryFor(entityType, id)` は「**この人**のスキル概況」「**このPJ**の WBS 進捗」のように**エンティティ1件**を主語にしたサマリーを返す。人・プロジェクト詳細への関連情報集約ビュー（Issue #83）が、他モジュールをハード参照せずに各枠を問い合わせるための任意契約（§9.5 の欠損時グレースフルの実証対象）。
...
- **出す条件はコアの述語 `MK.canOfferSample(id, targetId?)`**（`shared/core.js`・DOM 非依存）に集約する。`loadSample` / `exportData` / `importData` の3つを実装し、かつ `exportData(targetId)` の中身が空（`MK.isEmptyExport`）のときだけ `true`。未搭載・未実装・`exportData` が例外のいずれでも `false` を返し、モジュール画面を壊さない（§9.5 と同一原則）。
- **空判定に `summary().empty` は使わない**。`summary()` は**モジュール全体**を主語にする指標で、投入先とは粒度が合わない: scoped の wbs / metrics は全 PJ・全プロダクト横断で `empty` を出すため「他の PJ に1件あると空の PJ で出ない」、skills は People マスタが空かどうかまで見るため「人が1人いると永久に出ない」。見たいのは「**投入先が空か**」なので、投入先そのものである `exportData(targetId)` の中身で判定する。`MK.isEmptyExport(data)` は配列・オブジェクトの値がどれも空なら空とみなし、`version` / `uid` / `startTime` のようなスカラは数えない。
...
- **退避は「他の変更が入った時点で破棄する」**（CONVENTIONS §2.5-3）。投入直後の姿も一緒に覚えておき、現在のデータがそれと違えば――サンプルの上へ追記・編集した／JSON を取り込んだ／設定から一括サンプルを入れた／全削除した、のいずれでも――退避を捨てて片付け導線を引っ込める。片付けは `replace`（取り消し不能）なので、捨てないと**ユーザ自身が足したデータごと消す**。描画時と押下時の両方で確かめ、押下時に無効化されていたら実行せずその旨を伝える（無言の no-op にしない）。
- **「投入直後の姿」は mount が終わってから確定させる**。バーはモジュール本体より先に描くため、押した時点で採ると**描画中の自動投入・正規化を取りこぼす**（daily の `ensureDayInjected` はその日のルーチンを items へ投入して保存する）。取りこぼすとその差分で退避が即座に無効になり、そのモジュールだけ片付けが必ず失敗する。確定は `mount` が例外を投げても必ず行う（未確定の退避は上の破棄判定を素通りするため、確定を飛ばすと片付けがセッション中ずっと無条件で効く状態になり、以後に入れたデータを消す）。
- **投入しても何も入らないことがある**。`oneonone` は People が、`releases` は Product が1件も無いとサンプルを作れず空のまま返す。この場合は退避を残さず、足りないマスタを伝える（空のまま「サンプルを表示しています」へ切り替わると、入っていないのに片付け導線だけが出て投入をやり直せなくなる）。
- **押下時にも空か確かめ直す**（片付け側と対称）。バーはシェルの `main` 直下にあり**モジュール本体とは兄弟**なので、モジュールの `render()`（自分の root の作り直し）ではバーが消えない。バーが出たまま本体のフォームから入力できてしまうため、押下時に確かめないと全置換の `loadSample()` が入力したばかりのデータを消す。
- **`loadSample()` はユーザ設定を書き戻さない**。サンプルが差し替えてよいのはそのモジュールのレコードだけで、設定値（daily の `startTime` など）は保つ。項目が 0 件なら設定を変えていても「空」と判定されてバーが出るため（初回に最も起こりやすい）、書き戻すと投入した瞬間に設定が黙って既定へ戻る。
...
ゾーン分類（§1.4）に**直交する第2軸**として、「**同種のものを複数持つ／単一で持つ**」を統一的に扱う仕組みを定義する。「PJ 横断 / PJ 単位」軸を **Project 専用ではなく汎用の型**として扱う。

#### 3.7.1 スコープ次元と参照マスタ

- **スコープ次元（scope dimension）**: 「その1つの中に入って作業する対象」になれるマスタ。現状 **Project・Product**。複数持ちうるし単一のこともある。
...
- `scope: "global"` … 横断・常時（例: todo / resource）。既定。
- `scope: { dim: "project" }` … Project 文脈で動く（例: wbs / dashboard）。`{ dim: "product" }` も config 上は次元化済み（Issue #54）だが、宣言するモジュールはまだ無い。
...
- 利点: 対象単位のバックアップ／エクスポートがキー単位で完結、破損の影響を**対象単位に局所化**（§10.1）、配布プロファイルの「読み込まなければ物理的に存在しない」思想（§1.5）と一致。
...
- **所有と配線**: アロケーションは特定モジュールに属さない**中立な共有マスタ `mk:allocations`**（People / Projects と同格。§4.4）に置く。編集（planning）は**リソース（resource）**が担い、`ctx.allocations` 経由で読み書きする。
- 担当合計 vs アロケーションの**照合クロス機能**は未着手（§14）。

#### 3.7.6 型は汎用・次元化済みは Project / Product、モジュールは必要時に実体化（YAGNI）

- スコープ次元の**型／契約**は汎用に定義し、`MK_CONFIG.dimensions` への配線（次元化）は **Project・Product の2次元**。
- **Product マスタ**（`products`・`mk:products`）は People / Project と同格の横断マスタ（§4.4）で、`master: "products"` として Product 次元のマスタを兼ねる。
- **product-scoped モジュール**は metrics（プロダクト指標・§5）が担う（`scope: { dim: "product" }` を宣言。wbs が Project 次元でやるのと同じ要領）。次元の配線とモジュールの実体化は別軸で、以後のモジュールも追加時に `scope` を宣言すれば足りる。
- **ガードレール**: コード上で `"project"` を**決め打ち分岐しない**。次元は config の配列を回して扱う（[`CONVENTIONS.md`](CONVENTIONS.md)）。`shared/scope.js` / `shared/shell-core.js` は `MK_CONFIG.dimensions` を配列として汎用に走査するのみで、次元の追加で改修が要らないことをテスト（`test/scope.test.js` の Product ケース）で担保する。
...
> **本節の詳細は [`spec/import-migration.md`](spec/import-migration.md) §4.6 / §4.6.1 に分離。** UTF-8（BOM許容/出力はBOM付き）・RFC4180 自前パーサ・名前参照・寛容解釈で `shared/io.js` に集約。CSV 対象モジュールは §5 の一覧表（CSV 列＝✓）を正とする（マスタ側は People / Project / Product も対応）。共通ドメイン（People / Project）の列仕様、モジュール固有列（各 `spec/modules/<id>.md`）の参照先も同ファイルを正とする。名前参照は取込時に §8 名寄せで解決する。
...
| metrics | プロダクト指標（KGI/NSM/KPI・product-scoped） | プロダクト | [`spec/modules/metrics.md`](spec/modules/metrics.md) | – |
...
モジュールを**足す・外す・統合する**ときに常に満たす契約を定める（各モジュール内部の取り決めは各個別仕様に従う）。次に何を作るかの計画はここに置かない——このツールは統合という終点を過ぎて「用途に応じてモジュールを増減させて育てる」段階にあり、やることは Issue で決める（§14 は未決事項の置き場）。§9.1〜§9.4 は欠番（旧フェーズ節を刈った跡。番号での参照が多いため詰めない）。

### 9.5 モジュール着脱に耐える器

**原則**: **疎結合な任意契約の上で横断価値を出す。** 横断価値（人1人・プロジェクト1つを主語にした集約ビュー等）はモジュール間の結合を増やし、着脱耐性は結合を減らしたい。この緊張は「§3.6 / §3.6.1 の疎結合な任意契約を前提に置き、横断ビューがその契約の消費者＝実証になる」で解く（実証は人・プロジェクト詳細の集約ビュー＝§3.6.1・Issue #83）。本節は契約そのものではなく、**着脱の方針と保証**を定める。
...
統廃合は汎用マージエンジンを作らず、旧データ移行（§7.5）と同じ **one-off の移行スクリプト方式**で行う（`file://`・外部依存ゼロと整合）。前提は §9.5 の契約——「外してもデータは名前空間ごと生存し migrate できる」——であり、統廃合はその上に乗る。

**統廃合の判断記録**

モジュール／データ名前空間／マスタを統合・廃止するかの判断は、汎用の判断基準を先に作らず、**判断のたびに決定ログとして1行ずつ追記する**方式で育てる（下表）。

| 日付 | 対象 | 判断 | 理由 |
|---|---|---|---|
| 2026-07-07 | 人・プロジェクト詳細の集約ビュー（#83）と プロジェクト・ダッシュボード（#78・`dashboard`） | **統合せず役割分担で共存**。人詳細＝**マスタ側**（`master-people`）の集約ビューとしてシェルに実装（新規）。プロジェクトの集約は既存 `dashboard`（project-scoped モジュール）に一本化し、**master 側にプロジェクト詳細は作らない**（重複回避）。人・プロジェクトどちらも同じ任意契約 `summaryFor(entityType,id)`＋リーダ `MK.readEntitySummary` を土台にする。 | People は参照マスタ（スコープ次元にしない・§3.7.1）ゆえ「その中に入る」dashboard 型の scoped ビューに載せられず、マスタ一覧からの詳細が自然。一方 Project はスコープ次元で既に `dashboard` が「1 PJ を主語に WBS 進捗・アサイン・関連プロダクトを集約」する器を持つため、同じ表示を master 側に二重実装しない。両者を1つの汎用契約で賄うことで §9.5 の契約の実証を兼ねる。 |
...
### 9.7 モジュール間の直接参照（判断記録）

§9.5 が禁じるハード参照は**横断表示**（他モジュールの `summary()` / `summaryFor()` を覗きに行く経路）についてのもので、モジュールが自分の機能を成立させるために別モジュールの logic を参照すること自体は禁じていない。ただし参照はそのまま結合であり、**増やすか／どこまで許すかは判断を要する**。§9.6 と同じく汎用の基準を先に作らず、**判断のたびに決定ログとして1行ずつ追記する**方式で育てる（下表）。

参照を持つ側が常に満たす前提は2つ ── (1) 参照先が未搭載でも**動作を落として続行**する（`MK.logic.<id>` の存在チェック＋フォールバック。§9.5 の保証1）、(2) 参照先の**データ名前空間を直接読み書きしない**（logic の公開関数のみを通す）。

| 日付 | 対象 | 判断 | 理由 |
|---|---|---|---|
| 2026-08-16 | `daily` → `todo` の直接参照（`modules/daily/logic.js` の `todoOf` が `MK.logic.todo.tasks()` を線形探索し、`resolvedItems` 経由で項目数×タスク数になりうる） | **現状のまま許容する**（byId マップ・索引・専用の参照契約は導入しない）。再最適化は**実害＝件数増による体感遅延が観測されたとき**に別 Issue で検討する。 | daily の todo 由来項目は「実体は todo が持ち、daily の `done` / `title` は表示用スナップショット」という設計（`resolveDone` / `resolveItem`）で、正を引くには参照が要る。ここへ索引や中間契約を挟むと、**性能のためだけに daily↔todo の結合と同期対象を増やす**ことになり「モジュール依存を増やさない」を崩す。走査量は経路で異なる ── `dayItems` は先に date で絞るのでその日ぶんに限定されるが、`activeTodoIds` / `staleCount` / `summary` は `resolvedItems()` を**全日付の items** に対して呼ぶため、実質は「これまでの全項目数 × todo 件数」になる（`summary` は HOME を開くたび走る）。それでも想定規模（todo は数百〜千件・§10、daily の items は日々の積み上げ）では実測コストが体感に出ないため許容する。**実害の観測点はここ**（HOME の初期表示・デイリー画面の再描画）で、遅延が出たらまず `todoOf` の byId 化を検討する。**この判断は `/code-review` の同種指摘に対する却下根拠として参照してよい**（レビューは毎回コールドスタートで再提示されるため。CLAUDE.md 必須ルール4「既出の指摘は既出として却下してよい」）。 |
...
- 全体バックアップ（`scope:"all"` の JSON 書き出し）の実行日時を `mk:backup:v1` に記録し、設定画面に「最終バックアップ: n日前／未実施」を常時表示する。閾値（14日）超過または未実施のときは警告とエクスポート導線を出す（`MK.io.markBackup()` / `MK.io.backupFreshness()`）。鮮度に数えるのは全体バックアップのみで、部分エクスポート（モジュール／人／プロジェクト単位）は対象外。閾値を変える設定項目は持たない。記録は端末ごとの運用状態なので JSON エンベロープ（§4.2）には含めない（復元先の端末では「未実施」から始まる）。

### 10.2 アクセシビリティ・キーボード
- 主要操作はキーボードで到達可能、フォーカスを可視化、適切なラベル / `aria` を付与する。記号だけのコントロール（`⋯` / `×` / `↑` など）には `aria-label` を付ける ── **`title` は読み上げ名にならない**。文言がインライン編集で変わるものは文字列を焼かず `aria-labelledby` で id を指す（`modules/daily/view.js` の実装が手本）。
- コントラストは DESIGN トークン準拠（§6）。
- 既存ショートカット（todo のクイックキャプチャ Enter、編集 Esc キャンセル等）を尊重し、モジュール横断で一貫させる。
- 取り消しトースト（§6.3 `MK.ui.undoToast`）は表示中のみ **Ctrl+Z（Mac: Cmd+Z）** で取り消せる。トーストはページ末尾に生成されフォーカスも移さないため、ショートカットが唯一の現実的なキーボード導線になる（テキスト入力中は横取りせず、フォーカスがトースト内にある間は自動消滅しない）。

---

## 11. 受け入れ条件（統合観点）

統合として常に満たすべき条件（モジュールの列挙・CSV 対象などの「時点で変わる列挙」は §1.4・§4.6・§5 を正とする）。

1. `index.html` を `file://` で開くだけで、全モジュールに切替・操作できる（外部通信ゼロ）。
2. メンバーを「人の管理」に1回登録すると、People マスタを参照する全モジュールで同じメンバーとして参照される（再読込後も保持）。
3. 全体バックアップ（`scope:"all"`）を JSON で書き出し、別環境でインポートして全モジュール+マスタを復元できる。
4. CSV 対応モジュール・マスタ（§5）の CSV が共通規約（UTF-8 BOM・名前参照・寛容パース）で入出力でき、名前でマスタへ解決される。
5. 全モジュールが同一の DESIGN トークン・コンポーネントで描画され、見た目が統一されている。
6. 旧ツールの localStorage / 旧 JSON を取り込み、メンバー/プロジェクトが名寄せ統合される（§7）。
...
| 人 / プロジェクト | 別ストア・別管理画面に分離（軸が直交）。Project は軽量マスタに留める（[`spec/masters.md`](spec/masters.md) §4.4）。横断集約は master 側に作らず、プロジェクトは `dashboard`（#78）・人はマスタ詳細の集約ビュー（#83）が担う（§9.6 の判断記録）。 |
| ナビ分類（ゾーン） | アプリは「マネージャの作業机」。ナビを **自分** ＋ **EM が見る4領域（ピープル/デリバリー/プロダクト/テクノロジー）** で分類（§1.4）。現行ゾーン構成は `index.html` の `MK_CONFIG.zones` を正とする。マスタ管理（人/プロジェクト/プロダクト）はゾーンに属さず独立グループに置く（§6.4）。goals は自分ツールで Member/Project と結び付けない（KPI/OKR に見せない）。チーム OKR は将来別モジュール（§1.4）。 |
...
| 要員計画 | PJ 横断俯瞰は**共有アロケーション**（人×器×期間×%）を参照。WBS 担当とは別レコード（同じ People を指す）。アロケーションは中立な共有マスタ `mk:allocations` として実装し、WBS は独立維持（§3.7.5）。 |
...
| アプリ名 | **マネジメントくん**（技術名/ディレクトリ `management-kun`）。 |
| MVP | 基盤＋デザイン統一＋人/プロジェクトマスタ＋5モジュール移植＋旧データ自動移行（名寄せは自動作成）＋全体JSON入出力。名寄せレビュー/マスタ統合は MVP 外（未着手のものは §14）。CSV の現行対象は §4.6・§5 を正とする。 |
...
未決のものだけを挙げる（完了した項目は削除する。履歴は git を参照）。ラベルは **（未着手）** ＝やると決まっていて未実装のもの／**（将来）** ＝必要になったら検討するもの（いつやるかは決めない）。ラベルの無い項目は常時の運用課題（随時対応する）。

- （未着手）アプリ**アイコン**のデザイン（名前は確定: マネジメントくん）。
- 細かな表示調整（各モジュールの余白・レスポンシブの詰め）。共有層（`design.css`/`ui.js`）で対応する（CONVENTIONS.md）。
- （未着手）名寄せレビュー画面（§8.5）・マスタの事後マージ（§8.8）・「未解決の名前」一覧（§8.6）。
- （未着手）モジュール間クロス機能（例: WBS 担当合計 vs アロケーション照合）。複数モジュールを結合する横断機能のため、実装は必ず §9.5 の任意契約＋欠損時グレースフルの上に載せる。
- （将来）チームの OKR/KPI 管理。必要になれば goals とは別概念の新モジュールとして検討する（§1.4）。
- （将来）配布パッケージ生成（§1.5）。開示するモジュールを選び、必要な html / css / js のみ（マスタ管理・チーム系モジュールを含まない構成）を zip 等に詰めて出力する。ビルド不要・外部依存ゼロの制約下でどう実現するか（手順書方式か one-off スクリプトか）も含め、配布実績が生じたときに検討する（#222）。
