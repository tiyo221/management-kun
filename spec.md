...
**ピン留め指標の設計方針（行動につながる指標。Issue #172）**: `stats` に出す指標は「**その数字を見て次の一手が変わるか？**」を満たすものだけに絞る。

- **① 母数（総数・累計）は原則 `stats` に出さない**。全タスク数・目標数・記録数・技術数・メンバー数など「眺めるだけの数字」は排除する。
- **② 母数が文脈として要る場合も1つに留め、行動できる形へ言い換える**（例: 全タスク→未完＝残作業）。割合（達成率・カバー率など）は「今の状態」を表すので可。
- **③ `stats`＝定点の状態／`attention`＝いま対応が要る緊急事項、と役割を分ける**。同じ事実を両方へ二重表示しない（緊急は `attention` 帯に集約し、`stats` はそれ以外の状態を担う）。
- **④ `stats` は原則2つ（多くて3つ）**。カードを数字の羅列にしない。
- **scoped モジュール（wbs 等・§3.7.4）を HOME の1枚に畳むときは主語をエンティティ（PJ）側に寄せて件数化する**。タスク横断の混合平均のような「どれを見るべきか分からない数字」を避け、代表が要る要対応は `attention` で代表1件＋件数を名指しする（resource の過負荷表示と同型）。
- ゾーン単位の棚卸しは #202（self）/ #203（people）/ #204（delivery・wbs）/ #205（product）/ #206（tech）で実施する。

- `summary` 未実装・例外時も HOME は壊さない（カードは「開く」表示にフォールバック）。
- `attention` は **stats 同様に任意**。未実装・不正形式でも HOME は壊さない（帯に出ないだけ）。実装例: todo=期限切れ/今日期限、techstack=見直し期限の超過/接近、questions=停滞（長期未解決）、resource=過負荷（誰が過負荷か・warn）、wbs=期限超過（代表 PJ＋件数・error）、goals=期限超過、releases=遅延、skills=不足スキル、oneonone=期限切れアクション（#181 / #172）。
- 集計は logic 側の純関数に置き、テスト可能にする（`test/summary.test.js`）。`stats` の具体指標は方針①〜④に沿って各モジュールの個別 Issue（#202〜#206）で棚卸しするため、本節では全モジュール分を列挙せず二重管理を避ける。クリーンな例: todo=未完/期日未設定（母数の全タスクは出さず、attention の期限切れ/今日期限とも事実が重複しない＝方針③）。「今日」に依存する集計は基準日を引数で受け取れるようにする（決定的テスト。TESTING §1）。
...
| daily | デイリー（今日のタイムボクシング） | 自分 | [`spec/modules/daily.md`](spec/modules/daily.md) | – |
