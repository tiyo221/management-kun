...
- 全体バックアップ（`scope:"all"` の JSON 書き出し）の実行日時を `mk:backup:v1` に記録し、設定画面に「最終バックアップ: n日前／未実施」を常時表示する。閾値（14日）超過または未実施のときは警告とエクスポート導線を出す（`MK.io.markBackup()` / `MK.io.backupFreshness()`）。鮮度に数えるのは全体バックアップのみで、部分エクスポート（モジュール／人／プロジェクト単位）は対象外。閾値を変える設定項目は持たない。
