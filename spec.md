...
- 範囲: 本書は**全体統合仕様**。各モジュールの機能詳細は各ツールの既存 `spec.md` を正とし、本書はそれらの上位に立つ統合の取り決めを定義する。
...
### 1.3 非ゴール（やらないこと）
- エンティティの完全統合（タスクのスキーマを1つに強制する等）は行わない。各モジュール固有のデータ構造は独立を保つ（§5.4）。
...
| **チーム管理** | チーム/メンバー | skills / workload / wbs（＋人の管理・プロジェクト管理） | 使う |
...
メンバー・プロジェクトは**名前**で参照し、取込時に §8 の名寄せで解決する。各CSVは1行目をヘッダーとする。
...
| skills / ユーザ | `氏名, 役割, 備考` |
| skills / スキル | `大分類, 中分類, 小分類, コア, 目標レベル, 必要人数, 表示` |
| skills / 紐づけ | `メンバー名, 大分類, 中分類, 値`（縦持ち。値は `1〜5` / `-`） |
| wbs（エクスポート主体） | `WBS番号, タスク名, 担当者, 開始, 終了, 進捗, ステータス, 先行, 備考`（担当者は名前、先行は WBS 番号） |
...
各モジュールの**機能詳細は各ツールの既存 `spec.md` を正**とする。本節は統合上の位置づけ・共通マスタとの関係・固有データの所在のみ定義する。

### 5.1 goals — 目標みえるくん
- 役割: 大目標→ステップのロードマップ可視化 + 統計ダッシュボード。**個人ゾーン**のツール（自分の目標・§1.4）。
- 共通マスタ関係: **持たない**。Member も Project も参照しない（チームの KPI/OKR に見せないため）。
- 固有データ: `goals[]`（goal/step）。`mk:module:goals:v1`。
- 参照: [mokuhyo-mieru-kun/spec.md](../mokuhyo-mieru-kun/spec.md)

### 5.2 skills — スキル可視化
- 役割: メンバー×スキルのマトリクス・ギャップ・レーダー可視化。
- 共通マスタ関係: **Member を People マスタから参照**（従来の自前 members[] を廃し `people.members` を使用）。
- 固有データ: `skills[]`（スキルマスタ）、`ratings`（`"<memberId>:<skillId>"`）。`mk:module:skills:v1`。
- CSV: ユーザ（マスタ Member へ名寄せ）/ スキル / 紐づけの3種を共通CSV規約で入出力。
- 参照: [skill-mieru-kun/spec.md](../skill-mieru-kun/spec.md)

### 5.3 workload — タスク負荷可視化（旧 task-mieru-kun）
- 役割: メンバーの負荷×期間の中長期俯瞰、過負荷/低負荷シグナル、計画ベースライン。
- 共通マスタ関係: **Member を共通マスタから参照**。警告閾値（`capacityWarnHigh/Low`）・表示色はモジュール側 `memberSettings[memberId]` に保持。
- 固有データ: `tasks[]`（load/period/status）、`baseline`（スナップショット）、`memberSettings`。`mk:module:workload:v1`。
- 参照: [task-mieru-kun/spec.md](../task-mieru-kun/spec.md)

### 5.4 todo — GTD ToDo（旧 todo-kun）
- 役割: GTD ベースの軽量タスク管理。
- 共通マスタ関係: `project`（自由文字列）を **Project マスタへ名寄せ**（`projectId` 参照 + 後方互換の文字列保持）。担当者概念は持たない（個人用途のため）。
- 固有データ: `tasks[]`（status=inbox/next/waiting/someday/done、contexts、due 等）。`mk:module:todo:v1`。
- 参照: [todo-kun/spec.md](../todo-kun/spec.md)

### 5.5 wbs — WBS / ガント（旧 wbs-tool）
- 役割: 階層 WBS + ガント + 依存（FS）+ クリティカルパス（軽量版・backlog）。
- 共通マスタ関係: `assignee` 文字列を **Member 参照（`assigneeId`）へ移行**。Project マスタとの関連は任意。
- 固有データ: `tasks[]`（level/deps/progress/status 等）、`uid`。`mk:module:wbs:v1`。
- レイアウト: 統合アプリの幅（~950px）ではテーブルがガントより広くなるため、**テーブルの上下にガントを積む**構成とする（旧ツールの左右並び＋縦スクロール同期は、広幅向けの将来拡張）。ガントのバーにタスク名を載せ、単独でも読めるようにする。
- 参照: [wbs-tool/spec.md](../wbs-tool/spec.md)
...
2. モジュール切替は**2ゾーンに分けて表示**（§1.4）: 「**個人**」（ToDo / 目標）｜「**チーム管理**」（スキル / 負荷 / WBS ＋ 人の管理 / プロジェクト管理）。各ゾーンにラベルを付け、区切りで分離する。
3. メイン: 選択モジュールの描画領域。
4. **人の管理** / **プロジェクト管理**: それぞれ独立した管理画面（CRUD・名寄せ）。両者は別ドメインとして分離する。
...
### 7.1 旧データの取り込み
初回起動時、または「旧ツールから移行」操作で、旧 localStorage キー / 旧 JSON を検出して取り込む。

| 旧ソース | 旧キー / 形状 | 移行先 |
|---|---|---|
| 目標みえるくん | `mokuhyo-mieru-kun:v1` `{version,goals}` | `mk:module:goals:v1` |
| スキル可視化 | `skill-tool-data-v1` `{members,skills,ratings}` | members→`mk:people`、skills/ratings→`mk:module:skills:v1` |
| タスク可視化 | (旧キー) `{members,tasks,baseline}` | members→`mk:people`（閾値/色は memberSettings）、他→`mk:module:workload:v1` |
| todo-kun | `todo-kun.data.v1` `{version,exportedAt,tasks}` | `mk:module:todo:v1`、`project` 文字列→Project 名寄せ |
| WBS Tool | `wbs-tool-data-v1` `{uid,tasks}` ＋ 列幅 | `mk:module:wbs:v1`、`assignee`→Member 名寄せ |
...
| 個人 / チーム ゾーン | アプリは「マネージャの作業机」。ナビを **個人（todo/goals）** と **チーム管理（skills/workload/wbs＋人/プロジェクト）** に分離。goals は個人ツールで Member/Project と結び付けない（KPI/OKR に見せない）。チーム OKR は将来別モジュール（§1.4）。 |
