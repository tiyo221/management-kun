...
モジュールを**足す・外す・統合する**ときに常に満たす契約を定める（各モジュール内部の取り決めは各個別仕様に従う）。次に何を作るかの計画はここに置かない——このツールは統合という終点を過ぎて「用途に応じてモジュールを増減させて育てる」段階にあり、やることは Issue で決める（§14 は未決事項の置き場）。節番号の安定のため §9.1〜§9.4 は欠番とする。

### 9.5 モジュール着脱に耐える器

**原則**: **疎結合な任意契約の上で横断価値を出す。** 横断価値（人1人・プロジェクト1つを主語にした集約ビュー等）はモジュール間の結合を増やし、着脱耐性は結合を減らしたい。この緊張は「本節が疎結合な任意契約を定義し、横断ビューがその契約の消費者＝実証になる」で解く（実証は人・プロジェクト詳細の集約ビュー＝§3.6.1・Issue #83）。
...
未決のものだけを挙げる（完了した項目は削除する。履歴は git を参照）。
