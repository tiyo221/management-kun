...
| 日付 | 対象 | 判断 | 理由 |
|---|---|---|---|
| 2026-08-16 | `daily` → `todo` の直接参照（`modules/daily/logic.js` の `todoOf` が `MK.logic.todo.tasks()` を線形探索し、`resolvedItems` 経由で項目数×タスク数になりうる） | **現状のまま許容する**（byId マップ・索引・専用の参照契約は導入しない）。再最適化は**実害＝件数増による体感遅延が観測されたとき**に別 Issue で検討する。 | daily の todo 由来項目は「実体は todo が持ち、daily の `done` / `title` は表示用スナップショット」という設計（`resolveDone` / `resolveItem`）で、正を引くには参照が要る。ここへ索引や中間契約を挟むと、**性能のためだけに daily↔todo の結合と同期対象を増やす**ことになり「モジュール依存を増やさない」を崩す。想定規模（todo は数百〜千件・§10、daily は1日ぶんの項目）では線形探索の実測コストが問題にならず、`dayItems` は先に date で絞ってから解決するため走査量も限定される。**この判断は `/code-review` の同種指摘に対する却下根拠として参照してよい**（レビューは毎回コールドスタートで再提示されるため。CLAUDE.md 必須ルール4「既出の指摘は既出として却下してよい」）。 |
