...
- **プロダクトゾーン**は releases（リリース台帳）が担う。成果物そのものの台帳＝**Product マスタ（📦 プロダクト・§4.4）**はゾーンではなくシェルレベルの「マスタ」グループに人・プロジェクトと並び、§3.7 の Product スコープ次元のマスタを兼ねる（releases は横断モジュールで、product-scoped モジュールはまだ無い。判断は [`spec/modules/releases.md`](spec/modules/releases.md)）。
...
- 「機構」は作らない。担保は**規律＋テスト**: 横断表示は「任意契約（`summary()` / `summaryFor()`）で問い合わせ → 無ければ枠を省く」で書き、他モジュールへのハード参照を禁止する。`MK_CONFIG` からモジュールを外しても起動・全画面・横断表示が壊れないことは [`test/module-detach.test.js`](test/module-detach.test.js) が検証し、DOM 層は CONVENTIONS §6 の完成チェックリストで点検する。
...
未決のものだけを挙げる（完了した項目は削除する。履歴は git を参照）。ラベルは2種類: **（未着手）** ＝やると決まっていて未実装のもの／**（将来）** ＝必要になったら検討するもの（いつやるかは決めない）。
...
- （将来）product-scoped モジュールの実体化（§3.7.6。必要になったモジュール追加時の後続 Issue で扱う）。
