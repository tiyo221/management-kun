...
- 着脱耐性（外したモジュールがあっても起動・全画面・横断表示が壊れない）は §9.5 柱1・`test/module-detach.test.js` が担保する。
...
統合プロジェクトの観点でのフェーズ計画を定める（各モジュール内部の計画は各個別仕様に従う）。完了したフェーズの記述（MVP の内容・v1 時点の Phase 2 リスト等）は現在形へ刈り込み済みで、履歴は git を参照する（節番号の安定のため §9.1〜§9.4 は欠番とする）。v1 リストのうち未実装で残る項目（名寄せレビュー画面・マスタの事後マージ・「未解決の名前」一覧）は §14 に挙げる。

### 9.5 Phase 2 の方針（MVP 後の再定義・2026-07）

MVP 完了後、Phase 2 の柱を次のとおり定義した。
...
- モジュール間クロス機能（WBS 担当合計 vs アロケーション照合など）は柱2 ではなく Phase 3 の扱い（§9.6）。
...
**モジュール間クロス機能**: WBS 担当合計 vs アロケーション照合。複数モジュールを結合する横断機能であり、柱1の任意契約＋着脱耐性が固まってから着手する。実装は必ず任意契約＋欠損時グレースフルの上に載せる。
...
- （Phase 2）名寄せレビュー画面（§8.5）・マスタの事後マージ（§8.8）・「未解決の名前」一覧（§8.6）。
- （Phase 3）モジュール間クロス機能（例: 担当合計 vs アロケーション照合）（§9.6）。
