...
- **（思想確定・実装は段階進行）スコープ次元（§3.7）**: 「PJ 横断 / PJ 単位」軸の思想は §3.7 で確定済み。**実装済み**: シェルのスイッチャ・`ctx.scope`（#24）／wbs の対象別 namespace 化（#25）／workload の次元対応（共有アロケーション `人×器×期間×%`。#26）。**残る実装**: 要員計画モジュール（#27）・Product 実体化は後続 Issue で扱う。
