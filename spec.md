...
| **ピープル** | メンバー・チーム | skills / workload（＋人マスタ） | 使う |
| **デリバリー** | プロジェクトの遂行 | wbs（＋プロジェクトマスタ） | 使う |
...
- **当面の実装方針**: 新たな共有マスタは作らず（YAGNI）、既存 **workload を次元対応**させ `(memberId, targetId, period, %)` を持たせる。要員計画はその横断ビュー。第2の消費者が現れたら共有マスタへ昇格を検討。
...
2. モジュール切替は**ゾーンに分けて表示**（§1.4）: 「**自分**」（ToDo / 目標）｜「**ピープル**」（スキル / 負荷 ＋ 👤 人）｜「**デリバリー**」（WBS ＋ 📁 プロジェクト）。各ゾーンにラベルを付け、区切りで分離する（プロダクト/テクノロジーは現状モジュールが無いため表示しない）。
3. メイン: 選択モジュールの描画領域。
4. **マスタ管理**: 人・プロジェクトの管理画面。人とプロジェクトは**別ドメイン**（別ストア・別 CRUD・名寄せ）であり、ナビでも**ドメインに沿って別エントリ・別ビューに分離**する ── 「👤 人」を**ピープルゾーン**、「📁 プロジェクト」を**デリバリーゾーン**に置く（`master-people` / `master-projects` ビュー）。マスタは業務データであり、設定(config)とは混ぜない。
...
- **（思想確定・実装は後続）スコープ次元（§3.7）**: 「PJ 横断 / PJ 単位」軸の思想は §3.7 で確定済み。残る実装（シェルのスイッチャ・`ctx.scope`・wbs の対象別 namespace 化・workload の次元対応・要員計画モジュール・Product 実体化）は後続 Issue で扱う。
