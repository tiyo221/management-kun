...
- **「投入直後の姿」は mount が終わってから確定させる**。バーはモジュール本体より先に描くため、押した時点で採ると**描画中の自動投入・正規化を取りこぼす**（daily の `ensureDayInjected` はその日のルーチンを items へ投入して保存する）。取りこぼすとその差分で退避が即座に無効になり、そのモジュールだけ片付けが必ず失敗する。
